...
impl Feature {
    ...
    pub fn env_var_name(&self) -> String {
        format!("byterag_FEATURE_{}", self.as_str().to_uppercase())
    }
}
...
impl FeatureFlags {
    ...
    /// 파일에서 로드
    pub fn load_from_file(&self) -> ByteRagResult<()> {
    ...
    /// 파일에 저장
    pub fn save_to_file(&self) -> ByteRagResult<()> {
...
mod tests {
    ...
    fn test_feature_flag_env_var() {
        ...
        // 환경 변수 설정 (unsafe)
        unsafe {
            env::set_var("byterag_FEATURE_BINARY_SERIALIZATION", "true");
            env::set_var("byterag_FEATURE_MULTI_THREADING", "1");
            env::set_var("byterag_FEATURE_MVCC_EXTENSION", "false");
        }
        ...
        // 정리 (unsafe)
        unsafe {
            env::remove_var("byterag_FEATURE_BINARY_SERIALIZATION");
            env::remove_var("byterag_FEATURE_MULTI_THREADING");
            env::remove_var("byterag_FEATURE_MVCC_EXTENSION");
        }
    }
