...
- `ByteRagError::TransactionConflict` - Write-write conflict detected
- `ByteRagError::TransactionAborted` - Transaction was aborted
- `ByteRagError::InvalidTransactionState` - Invalid state transition

**Example:**
```rust
match tx.commit() {
    Ok(_) => println!("Committed"),
    Err(ByteRagError::TransactionConflict) => {
