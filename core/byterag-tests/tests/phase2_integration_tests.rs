...
use byterag_core::engine::{BenchmarkRunner, Feature, FeatureFlags};
...
use byterag_core::wal::partitioned_wal::{ParallelCheckpointManager, PartitionedWalWriter};
...
#[test]
fn test_query_plan_cache_integration() -> ByteRagResult<()> {
...
#[test]
fn test_zero_downtime_ddl() -> ByteRagResult<()> {
...
/// 테스트 4: WAL 병렬 쓰기
#[test]
fn test_wal_parallel_write() -> ByteRagResult<()> {
    let dir = tempdir().unwrap();
...
/// 테스트 5: 체크포인트 직렬화
#[test]
fn test_checkpoint_serialization() -> ByteRagResult<()> {
    let dir = tempdir().unwrap();
    let wal = PartitionedWalWriter::new(dir.path().to_path_buf(), 100)?;
...
#[test]
fn test_schema_versioning() -> ByteRagResult<()> {
    let mgr = SchemaVersionManager::new();
...
#[test]
fn test_index_versioning() -> ByteRagResult<()> {
...
/// 테스트 8: 전체 최적화 스택 통합
#[test]
fn test_full_optimization_stack() -> ByteRagResult<()> {
    let dir = tempdir().unwrap();
...
/// 테스트 9: 성능 회귀 방지
#[test]
fn test_rollback_on_regression() -> ByteRagResult<()> {
...
/// 부하 테스트: WAL 대량 쓰기
#[test]
fn test_wal_stress_write() -> ByteRagResult<()> {
    let dir = tempdir().unwrap();
...
/// 안정성 테스트: 스키마 대량 변경
#[test]
fn test_schema_stability() -> ByteRagResult<()> {
    let mgr = SchemaVersionManager::new();
...
#[test]
fn test_concurrent_stability() -> ByteRagResult<()> {
    use std::thread;
