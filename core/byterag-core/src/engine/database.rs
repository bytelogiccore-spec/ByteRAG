...
use crate::monitoring::ByteRagMetrics;
...
pub struct Database {
    ...
    pub(crate) metrics: Arc<ByteRagMetrics>,
...
impl Database {
    ...
    /// assert!(text.contains("byterag_inserts_total 1"));
    /// # Ok(())
    /// # }
    /// ```
    ...
    ///
    /// ```rust,no_run
    /// use byterag_core::{Database, engine::{StreamIngester, StreamEvent}};
