...
**파일:**
- 수정: `core/byterag-core/src/sql/view.rs`
...
```bash
cargo test -p byterag-core test_materialized_view_cache -- --nocapture
```
...
impl MaterializedViewRegistry {
    ...
    pub fn create(&self, name: &str, sql: &str, refresh_interval_secs: Option<u64>) -> ByteRagResult<()> {
    ...
    pub fn set_cache(&self, name: &str, batches: Vec<RecordBatch>) -> ByteRagResult<()> {
        let entry = self.views.get(&name.to_lowercase())
            .ok_or_else(|| ByteRagError::InvalidArguments(format!("'{}' 뷰를 찾을 수 없음", name)))?;
...
```bash
cargo test -p byterag-core test_materialized_view_cache
```

**Step 5: 커밋**

```bash
git add core/byterag-core/src/sql/view.rs
...
**파일:**
- 수정: `core/byterag-core/src/engine/database.rs`
- 수정: `core/byterag-core/src/engine/constructors.rs`
- 수정: `core/byterag-core/src/sql/interface.rs`
...
fn handle_create_materialized_view(&self, sql: &str) -> ByteRagResult<Vec<RecordBatch>> {
    ...
    let as_pos = after.to_uppercase().find(" AS ")
        .ok_or_else(|| ByteRagError::SqlParse { message: "CREATE MATERIALIZED VIEW requires AS".into(), sql: sql.to_string() })?;
...
```bash
cargo test -p byterag-core -- mat
git add core/byterag-core/src/
...
```bash
cargo test -p byterag-core -- matview
git add core/byterag-core/src/
...
**파일:**
- 생성: `core/byterag-core/src/engine/stream_ingester.rs`
- 수정: `core/byterag-core/src/engine/mod.rs`
...
```bash
cargo test -p byterag-core test_stream_ingester_insert_update_delete
```
...
```bash
cargo test -p byterag-core test_stream_ingester_insert_update_delete
git add core/byterag-core/src/engine/stream_ingester.rs core/byterag-core/src/engine/mod.rs
...
```bash
cargo test -p byterag-core -- stream
git add core/byterag-core/src/
