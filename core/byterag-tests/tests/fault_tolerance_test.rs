use byterag_core::error::ByteRagError;
use byterag_core::grid::manager::GridManager;
use byterag_core::grid::quic::QuicChannel;
use byterag_core::sharding::router::ShardRouter;
use byterag_core::sql::executor::distributed_executor::DistributedExecutor;
use byterag_core::sql::executor::local_executor::LocalExecutor;
use byterag_core::sql::planner::types::PhysicalPlan;
use byterag_core::storage::erasure_coding::distributed_store::DistributedErasureCodingStore;
use byterag_core::storage::erasure_coding::store::ErasureCodingStore;
...
async fn test_fault_tolerance_timeout_retry() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter("byterag_core=info,fault_tolerance_test=debug")
        .try_init();
    ...
    unsafe {
        std::env::set_var("byterag_WORKER_TIMEOUT_SECS", "1");
    }
    ...
    let registry = Arc::new(byterag_core::storage::metadata::MetadataRegistry::new());
    ...
    let plan = PhysicalPlan::HashJoin {
        ...
        on: vec![(0, 0)],
        join_type: byterag_core::sql::planner::types::JoinType::Inner,
    };
