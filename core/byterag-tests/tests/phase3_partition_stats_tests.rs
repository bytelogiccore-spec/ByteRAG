...
use byterag_core::storage::partition::{PartitionMap, PartitionStats, PartitionType};

#[test]
fn test_partition_stats_update_and_query() -> ByteRagResult<()> {
...
#[test]
fn test_all_partition_stats() -> ByteRagResult<()> {
    let db = Database::open_in_memory()?;
...
#[test]
fn test_partition_stats_missing_returns_error() -> ByteRagResult<()> {
    let db = Database::open_in_memory()?;
...
#[test]
fn test_partition_stats_update_overwrites() -> ByteRagResult<()> {
    let db = Database::open_in_memory()?;
