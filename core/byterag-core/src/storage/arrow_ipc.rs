...
/// - 95% faster than JSON
pub fn write_ipc_batch(batch: &RecordBatch) -> ByteRagResult<Vec<u8>> {
    let mut buffer = Vec::new();

    {
        let mut writer = writer::FileWriter::try_new(&mut buffer, &batch.schema())
            .map_err(|e| ByteRagError::Storage(format!("Arrow IPC write error: {}", e)))?;

        writer
            .write(batch)
            .map_err(|e| ByteRagError::Storage(format!("Arrow IPC batch write error: {}", e)))?;

        writer
            .finish()
            .map_err(|e| ByteRagError::Storage(format!("Arrow IPC finish error: {}", e)))?;
    }
...
/// - No parsing overhead
pub fn read_ipc_batch(bytes: &[u8]) -> ByteRagResult<RecordBatch> {
    let cursor = Cursor::new(bytes);

    let mut reader = reader::FileReader::try_new(cursor, None)
        .map_err(|e| ByteRagError::Storage(format!("Arrow IPC read error: {}", e)))?;

    let batch = reader
        .next()
        .ok_or_else(|| ByteRagError::Storage("No batch in Arrow IPC file".to_string()))?
        .map_err(|e| ByteRagError::Storage(format!("Arrow IPC batch read error: {}", e)))?;
