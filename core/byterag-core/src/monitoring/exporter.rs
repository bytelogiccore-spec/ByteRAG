...
//!
//! Converts `ByteRagMetrics` to Prometheus text format (version 0.0.4).
//! See: https://prometheus.io/docs/instrumenting/exposition_formats/

use crate::monitoring::metrics::ByteRagMetrics;

/// Export all ByteRAG metrics in Prometheus text exposition format.
///
/// # Returns
...
/// # Example output
/// ```text
/// # HELP byterag_inserts_total Total INSERT operations
/// # TYPE byterag_inserts_total counter
/// byterag_inserts_total 1234
/// ```
pub fn export_prometheus(metrics: &ByteRagMetrics) -> String {
    ...
    append_counter(
        &mut out,
        "byterag_deletes_total",
        snap.deletes_total,
        "Total number of DELETE operations",
    );
    append_counter(
        &mut out,
        "byterag_sql_queries_total",
    ...
    append_counter(
        &mut out,
        "byterag_delta_hits_total",
        snap.delta_hits,
        "Cache hits in Tier 1 (Delta Store)",
    );
    append_counter(
        &mut out,
        "byterag_delta_misses_total",
        snap.delta_misses,
        "Cache misses in Tier 1 (Delta Store)",
    );
    append_gauge_f64(
        &mut out,
        "byterag_delta_hit_rate",
        snap.delta_hit_rate,
    ...
    append_counter(
        &mut out,
        "byterag_cache_hits_total",
        snap.cache_hits,
        "Cache hits in Tier 2 (Columnar Cache)",
    );
    append_counter(
        &mut out,
        "byterag_cache_misses_total",
        snap.cache_misses,
        "Cache misses in Tier 2 (Columnar Cache)",
    );
    append_gauge_f64(
        &mut out,
        "byterag_cache_hit_rate",
        snap.cache_hit_rate,
    ...
    append_counter(
        &mut out,
        "byterag_partition_prune_hits_total",
...
mod tests {
    ...
    fn test_prometheus_format() {
        ...
        assert!(
            output.contains("byterag_inserts_total 2"),
            "inserts_total should be 2"
        );
        assert!(
            output.contains("byterag_sql_queries_total 1"),
            "sql_queries_total should be 1"
        );
        assert!(output.contains("# TYPE byterag_inserts_total counter"));
        assert!(output.contains("# TYPE byterag_cache_hit_rate gauge"));
        assert!(output.contains("# TYPE byterag_query_latency_us histogram"));
        assert!(output.contains("byterag_query_latency_us_count 1"));
    }
