...
impl SchemaVersionManager {
    ...
    /// 초기 스키마 등록
    pub fn register_table(&self, table: &str, schema: Arc<Schema>) -> ByteRagResult<u64> {
    ...
    pub fn alter_table(
        ...
        let mut history = self
            .versions
            .get_mut(table)
            .ok_or_else(|| ByteRagError::TableNotFound(table.to_string()))?;
    ...
    pub fn get_current(&self, table: &str) -> ByteRagResult<Arc<Schema>> {
        self.current_cache
            .get(table)
            .map(|r| r.value().clone())
            .ok_or_else(|| ByteRagError::TableNotFound(table.to_string()))
    ...
    pub fn get_at_version(&self, table: &str, version: u64) -> ByteRagResult<Arc<Schema>> {
        let history = self
            .versions
            .get(table)
            .ok_or_else(|| ByteRagError::TableNotFound(table.to_string()))?;
        ...
        history
            ...
            .ok_or_else(|| {
                ByteRagError::Serialization(format!("Version {version} not found for {table}"))
            })
    }

    /// 스키마 버전 히스토리 조회
    pub fn version_history(&self, table: &str) -> ByteRagResult<Vec<SchemaVersion>> {
        self.versions
            .get(table)
            .map(|r| r.value().clone())
            .ok_or_else(|| ByteRagError::TableNotFound(table.to_string()))
    ...
    /// 현재 버전 번호 조회
    pub fn current_version(&self, table: &str) -> ByteRagResult<u64> {
    ...
    /// 스키마 롤백 (이전 버전으로)
    pub fn rollback(&self, table: &str, target_version: u64) -> ByteRagResult<()> {
