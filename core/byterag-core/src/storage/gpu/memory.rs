...
impl<T: Clone + cudarc::driver::DeviceRepr> UnifiedBuffer<T> {
    ...
    pub fn new(device: Arc<CudaContext>, size: usize) -> ByteRagResult<Self> {
    ...
    pub fn from_vec(device: Arc<CudaContext>, data: Vec<T>) -> ByteRagResult<Self> {
        let size = data.len();
        Ok(Self {
            device,
            data,
    ...
    pub fn prefetch_to_gpu(&mut self) -> ByteRagResult<()> {
        ...
        if !self.prefetched {
            ...
            let _gpu_slice = stream
                ...
                .map_err(|e| ByteRagError::Gpu(format!("Prefetch failed: {:?}", e)))?;
    ...
    pub fn prefetch_to_cpu(&mut self) -> ByteRagResult<()> {
    ...
    pub fn to_device(&self) -> ByteRagResult<CudaSlice<T>> {
        let stream = self.device.default_stream();
        stream
            .clone_htod(&self.data)
            .map_err(|e| ByteRagError::Gpu(format!("Upload failed: {:?}", e)))
    }
