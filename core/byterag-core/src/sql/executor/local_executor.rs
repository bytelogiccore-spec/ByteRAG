...
type ShuffleReceiver = tokio::sync::mpsc::Receiver<ByteRagResult<Option<Vec<u8>>>>;
...
pub struct DistributedChannels {
    ...
    pub exchanges: HashMap<usize, mpsc::Sender<ByteRagResult<Option<Vec<u8>>>>>,
...
impl LocalExecutor {
    ...
    pub fn execute_collect(&self, plan: &PhysicalPlan) -> ByteRagResult<Vec<RecordBatch>> {
    ...
    pub fn build_operator(&self, plan: &PhysicalPlan) -> ByteRagResult<Box<dyn PhysicalOperator>> {
        ...
        match plan {
            PhysicalPlan::TableScan {
                ...
                let schema = schemas
                    .get(table)
                    .cloned()
                    .ok_or_else(|| ByteRagError::TableNotFound(table.clone()))?;
                drop(schemas);
                drop(store);
            ...
            other => Err(ByteRagError::SqlNotSupported {
                feature: format!(
