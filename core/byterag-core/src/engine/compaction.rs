...
impl Compactor {
    ...
    pub fn bypass_flush(db: &Database, table: &str) -> ByteRagResult<()> {
        if let DeltaVariant::Columnar(delta) = &db.delta {
            ...
            if let Err(e) = std::fs::create_dir_all(&ros_dir) {
                return Err(crate::error::ByteRagError::Storage(format!(
    ...
    pub fn bypass_flush_tables(db: &Database, tables: &[&str]) -> Vec<ByteRagResult<()>> {
        tables
            .par_iter()
