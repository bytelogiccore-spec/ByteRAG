...
impl ParallelSqlParser {
    ...
    pub fn parse(&self, sql: &str) -> ByteRagResult<Vec<Statement>> {
        Parser::parse_sql(&self.dialect, sql).map_err(|e| ByteRagError::SqlParse {
            message: e.to_string(),
            sql: sql.to_string(),
        })
    }
    ...
    ///
    /// ```rust
    /// use byterag_core::sql::parallel_parser::ParallelSqlParser;
    ///
    ...
    /// ```
    pub fn parse_batch(&self, sqls: &[&str]) -> ByteRagResult<Vec<Vec<Statement>>> {
        let len = sqls.len();
        if len == 0 {
            return Ok(Vec::new());
        }
    ...
    fn parallel_parse_adaptive(
        ...
        sqls: &[&str],
        avg_complexity: f64,
    ) -> Vec<Option<ByteRagResult<Vec<Statement>>>> {
        let len = sqls.len();
        ...
        if avg_complexity > 5.0 {
            ...
            let mut results: Vec<Option<ByteRagResult<Vec<Statement>>>> = Vec::with_capacity(len);
            results.resize_with(len, || None);

            // Use parallel chunks with index tracking
            let chunk_results: Vec<(usize, Vec<ByteRagResult<Vec<Statement>>>)> = sqls
                .par_chunks(chunk_size)
                .enumerate()
                .map(|(chunk_idx, chunk)| {
                    let start_idx = chunk_idx * chunk_size;
                    let parsed: Vec<ByteRagResult<Vec<Statement>>> =
    ...
    pub fn parse_batch_partial(
        &self,
        sqls: &[&str],
    ) -> (Vec<Vec<Statement>>, Vec<(usize, ByteRagError)>) {
    ...
    pub fn parse_batch_with_callback<F>(&self, sqls: &[&str], mut callback: F) -> ByteRagResult<()>
    where
        F: FnMut(usize, ByteRagResult<Vec<Statement>>) -> ByteRagResult<()>,
    {
        let results = if let Some(pool) = &self.thread_pool {
            pool.install(|| {
                sqls.par_iter()
