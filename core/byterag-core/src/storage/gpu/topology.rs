...
impl DeviceTopology {
    ...
    pub fn detect() -> ByteRagResult<Self> {
        // Get device count
        let device_count = CudaContext::device_count()
            .map_err(|e| ByteRagError::Gpu(format!("Failed to get device count: {:?}", e)))?;

        if device_count == 0 {
            return Err(ByteRagError::Gpu("No CUDA devices found".to_string()));
        }
        ...
        for i in 0..device_count {
            let device = CudaContext::new(i as usize).map_err(|e| {
                ByteRagError::Gpu(format!("Failed to initialize device {}: {:?}", i, e))
            })?;
            devices.push(device);
        }
    ...
    pub fn enable_peer_access(&self, i: usize, j: usize) -> ByteRagResult<()> {
        if i >= self.device_count || j >= self.device_count {
            return Err(ByteRagError::Gpu(format!(
