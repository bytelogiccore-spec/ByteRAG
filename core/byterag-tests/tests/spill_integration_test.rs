...
use byterag_core::sql::executor::SpillContext;
use byterag_core::sql::executor::{HashAggregateOperator, HashJoinOperator, PhysicalOperator};
use byterag_core::sql::planner::{AggregateFunction, AggregateMode, JoinType, PhysicalAggExpr};
use std::sync::Arc;
...
impl PhysicalOperator for VecOperator {
    ...
    fn next(&mut self) -> byterag_core::error::ByteRagResult<Option<RecordBatch>> {
    ...
    fn reset(&mut self) -> byterag_core::error::ByteRagResult<()> {
        self.idx = 0;
        Ok(())
    }
}
