...
impl Scheduler {
    ...
    /// 작업 등록
    pub fn register(&self, job: ScheduledJob) -> ByteRagResult<()> {
        ...
        if jobs.contains_key(&name) {
            return Err(crate::error::ByteRagError::DuplicateCallable(name));
        }
    ...
    /// 실행 준비된 작업 확인 및 실행
    pub fn tick(&self, ctx: &ExecutionContext) -> ByteRagResult<Vec<String>> {
        let mut jobs = self
            .jobs
            .write()
            .map_err(|_| crate::error::ByteRagError::LockPoisoned)?;
