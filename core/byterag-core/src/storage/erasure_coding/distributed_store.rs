...
impl DistributedErasureCodingStore {
    ...
    pub async fn store(&self, key: &str, data: &[u8]) -> ByteRagResult<()> {
    ...
    pub async fn retrieve(&self, key: &str) -> ByteRagResult<Option<Vec<u8>>> {
        // 로컬 조회 시도 (가장 빠름)
    ...
    /// EC 데이터 삭제
    pub async fn delete(&self, key: &str) -> ByteRagResult<bool> {
        // 모든 노드에 DeleteShard 요청 전송 확장 가능
        self.local_store.delete(key)
    }

    /// 로컬 스토어에 단일 샤드 기록 (GridManager에서 호출)
    pub fn local_store_shard(&self, key: &str, shard_id: usize, data: &[u8]) -> ByteRagResult<()> {
        self.local_store.store_shard(key, shard_id, data)
    }

    /// 로컬 스토어에서 단일 샤드 조회 (GridManager에서 호출)
    pub fn local_fetch_shard(&self, key: &str, shard_id: usize) -> ByteRagResult<Option<Vec<u8>>> {
