...
```c
// 기본 INSERT
byterag_execute_sql(db, "INSERT INTO users VALUES (1, 'Alice', 'alice@example.com', 25)");
...
// WHERE 조건
result = byterag_execute_sql(db, "SELECT * FROM users WHERE age >= 18");
if (result) {
    printf("Adults: %s\n", result);
    byterag_free_string(result);
}
...
// ORDER BY
result = byterag_execute_sql(db, "SELECT * FROM users ORDER BY age DESC");
if (result) {
    printf("Sorted: %s\n", result);
    byterag_free_string(result);
}
...
// LIMIT
result = byterag_execute_sql(db, "SELECT * FROM users LIMIT 10");
if (result) {
    printf("Top 10: %s\n", result);
    byterag_free_string(result);
}
...
```c
// 단일 컬럼 수정
byterag_execute_sql(db, "UPDATE users SET age = 26 WHERE id = 1");
...
// 조건부 수정
byterag_execute_sql(db, "UPDATE users SET age = age + 1 WHERE age < 30");
```
...
// 조건부 삭제
byterag_execute_sql(db, "DELETE FROM users WHERE age < 18");

// 전체 삭제 (주의!)
byterag_execute_sql(db, "DELETE FROM users");
```
...
// SQL 실행
if (byterag_execute_sql(db, "INSERT INTO users VALUES (1, 'Alice', 'alice@example.com', 25)") == NULL) {
    byterag_rollback(tx);
    fprintf(stderr, "Insert failed\n");
    return 1;
}

if (byterag_execute_sql(db, "INSERT INTO users VALUES (2, 'Bob', 'bob@example.com', 30)") == NULL) {
    byterag_rollback(tx);
    fprintf(stderr, "Insert failed\n");
    return 1;
}
...
UserManager* user_manager_create(const char* db_path) {
    ...
    mgr->db = byterag_open(db_path);
    
    // 스키마 초기화
    byterag_execute_sql(mgr->db,
...
int user_manager_create_user(UserManager* mgr, const char* username, const char* email) {
    ...
    char* result = byterag_execute_sql(mgr->db, sql);
    if (result) {
        byterag_free_string(result);
        return user_id;
    }
...
char* user_manager_get_user(UserManager* mgr, int user_id) {
    ...
    return byterag_execute_sql(mgr->db, sql);
}

void user_manager_destroy(UserManager* mgr) {
    byterag_close(mgr->db);
    free(mgr);
}
