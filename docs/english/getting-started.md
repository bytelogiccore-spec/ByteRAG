...
Since v0.2.0, all examples are separated into the `byterag-examples` package. You can run them using the following commands:

```bash
# Run basic quick-start example
cargo run -p byterag-examples --example quick_start

# Run distributed grid server example
cargo run -p byterag-examples --example distributed_grid

# Run 5-Tier storage tiering example
cargo run -p byterag-examples --example storage_tiering
```
...
```toml
[dependencies]
byterag-core = { version = "0.2.0-beta", features = ["gpu"] }
```
