...
impl ParallelQueryExecutor {
    ...
    ) -> ByteRagResult<AggregateResult> {
        if batches.is_empty() {
            return Ok(AggregateResult::empty(agg_type));
        }

        // 행 수 기반 순차/병렬 분기
        let partials: Vec<ByteRagResult<PartialAggregate>> = if self.should_parallelize(batches) {
    ...
    fn apply_filter_to_batch(
        ...
        let mask = result
            ...
            .ok_or_else(|| crate::error::ByteRagError::TypeMismatch {
    ...
    fn partial_aggregate(
        ...
        column_idx: usize,
        agg_type: AggregateType,
    ) -> ByteRagResult<PartialAggregate> {
