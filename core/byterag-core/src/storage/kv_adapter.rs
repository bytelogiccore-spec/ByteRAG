...
pub fn kv_to_batch(rows: Vec<(Vec<u8>, Vec<u8>)>) -> ByteRagResult<RecordBatch> {
    if rows.is_empty() {
...
pub fn batch_to_kv(batch: &RecordBatch) -> ByteRagResult<Vec<(Vec<u8>, Vec<u8>)>> {
    if batch.num_rows() == 0 {
        return Ok(Vec::new());
    }
    ...
    let key_array = batch
        ...
        .ok_or_else(|| ByteRagError::Schema("Key column is not Binary".to_string()))?;

    let value_array = batch
        .column(1)
        .as_any()
        .downcast_ref::<BinaryArray>()
        .ok_or_else(|| ByteRagError::Schema("Value column is not Binary".to_string()))?;
...
pub fn merge_batches(batches: Vec<Arc<RecordBatch>>) -> ByteRagResult<RecordBatch> {
    if batches.is_empty() {
        return Ok(RecordBatch::new_empty(kv_schema()));
    }
