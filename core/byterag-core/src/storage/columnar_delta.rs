...
fn find_key_in_batch(batch: &RecordBatch, target_key: &[u8]) -> ByteRagResult<Option<Vec<u8>>> {
    if batch.num_rows() == 0 {
        return Ok(None);
    }
    ...
    let key_array = batch
        ...
        .ok_or_else(|| ByteRagError::Storage("Key column is not BinaryArray".into()))?;
    ...
    for i in 0..key_array.len() {
        if !key_array.is_null(i) && key_array.value(i) == target_key {
            ...
            let value_array = batch
                ...
                .as_any()
                .downcast_ref::<BinaryArray>()
                .ok_or_else(|| ByteRagError::Storage("Value column is not BinaryArray".into()))?;
...
fn sort_batch_by_key(batch: &RecordBatch) -> ByteRagResult<RecordBatch> {
    if batch.num_rows() == 0 {
        return Ok(batch.clone());
    }
...
impl StorageBackend for ColumnarDelta {
    ...
    fn insert_batch(&self, table: &str, rows: Vec<(Vec<u8>, Vec<u8>)>) -> ByteRagResult<()> {
        if rows.is_empty() {
            return Ok(());
        }
    ...
    fn delete(&self, table: &str, key: &[u8]) -> ByteRagResult<bool> {
    ...
    fn count(&self, table: &str) -> ByteRagResult<usize> {
        let batches = self.get_visible_batches(table, u64::MAX);
