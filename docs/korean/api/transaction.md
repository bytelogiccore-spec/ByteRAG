...
### `Database::begin() -> ByteRagResult<Transaction<'_, Active>>`
...
### `commit(self) -> ByteRagResult<Transaction<'_, Committed>>`
...
### `abort(self) -> ByteRagResult<Transaction<'_, Aborted>>`
...
- `ByteRagError::TransactionConflict`: 같은 데이터에 대해 동시에 쓰기가 발생하여 충돌이 일어난 경우 (재시도 필요)
- `ByteRagError::TransactionAborted`: 트랜잭션이 중단된 상태에서 작업을 시도한 경우
