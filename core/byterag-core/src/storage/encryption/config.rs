...
//!
//! ```rust
//! use byterag_core::storage::encryption::{EncryptionConfig, EncryptionAlgorithm};
//!
...
impl EncryptionConfig {
    ...
    pub fn encrypt(&self, plaintext: &[u8]) -> ByteRagResult<Vec<u8>> {
        ...
        let ciphertext = match self.algorithm {
            EncryptionAlgorithm::Aes256GcmSiv => {
                ...
                cipher.encrypt(nonce, plaintext).map_err(|e| {
                    ByteRagError::Encryption(format!("AES-GCM-SIV encrypt failed: {}", e))
                })?
            }
            EncryptionAlgorithm::ChaCha20Poly1305 => {
    ...
    /// - Wrong key used
    pub fn decrypt(&self, encrypted: &[u8]) -> ByteRagResult<Vec<u8>> {
        if encrypted.len() < NONCE_SIZE {
            return Err(ByteRagError::Encryption(
    ...
    pub fn encrypt_with_aad(&self, plaintext: &[u8], aad: &[u8]) -> ByteRagResult<Vec<u8>> {
        ...
        let ciphertext = match self.algorithm {
            EncryptionAlgorithm::Aes256GcmSiv => {
                ...
                cipher.encrypt(nonce, payload).map_err(|e| {
                    ByteRagError::Encryption(format!("AES-GCM-SIV encrypt+AAD failed: {}", e))
                })?
            }
            EncryptionAlgorithm::ChaCha20Poly1305 => {
                let cipher = ChaCha20Poly1305::new(GenericArray::from_slice(&self.key));
                cipher.encrypt(nonce, payload).map_err(|e| {
                    ByteRagError::Encryption(format!("ChaCha20 encrypt+AAD failed: {}", e))
                })?
            }
        };
    ...
    pub fn decrypt_with_aad(&self, encrypted: &[u8], aad: &[u8]) -> ByteRagResult<Vec<u8>> {
        use aes_gcm_siv::aead::Payload;
        ...
        match self.algorithm {
            EncryptionAlgorithm::Aes256GcmSiv => {
                ...
                cipher.decrypt(nonce, payload).map_err(|e| {
                    ByteRagError::Encryption(format!("AES-GCM-SIV decrypt+AAD failed: {}", e))
                })
            }
            EncryptionAlgorithm::ChaCha20Poly1305 => {
                let cipher = ChaCha20Poly1305::new(GenericArray::from_slice(&self.key));
                cipher.decrypt(nonce, payload).map_err(|e| {
                    ByteRagError::Encryption(format!("ChaCha20 decrypt+AAD failed: {}", e))
                })
            }
        }
    }
