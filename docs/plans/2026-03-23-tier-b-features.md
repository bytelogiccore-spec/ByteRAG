...
**Date**: 2026-03-23
**Target**: `byterag-core` v0.3.0 (Tier A 완료 후)
...
[dependencies]
byterag-core = { path = "../byterag-core" }
...
**Files:**
- Modify: `core/byterag-core/src/wal/mod.rs` — `WalEntry`에 `timestamp_ns: u64` 필드 추가
...
**Files:**
- Create: `core/byterag-core/src/wal/timestamp_index.rs`
- Test: inline tests
...
impl WalTimestampIndex {
    ...
    /// WAL 파일에서 인덱스 빌드
    pub fn build_from_wal(wal_dir: &Path) -> ByteRagResult<Self> {
...
**Files:**
- Create: `core/byterag-core/src/engine/pitr.rs`
- Modify: `core/byterag-core/src/engine/mod.rs` — `pub mod pitr;`
- Test: inline tests

```rust
impl Database {
    /// 특정 시점으로 복구된 읽기 전용 Database 스냅샷 반환
    pub fn recover_to_timestamp(&self, timestamp_ns: u64) -> ByteRagResult<Database> {
    ...
    /// ISO 8601 문자열로 PITR
    pub fn recover_to_point(&self, iso_datetime: &str) -> ByteRagResult<Database> {
...
**Files:**
- Create: `core/byterag-core/src/grid/placement.rs`
- Modify: `core/byterag-core/src/grid/mod.rs` — `pub mod placement;`
- Test: inline tests
...
impl Database {
    pub fn set_placement_rule(&self, table: &str, rule: PlacementRule) -> ByteRagResult<()> { ... }
    pub fn get_placement_rule(&self, table: &str) -> ByteRagResult<Option<PlacementRule>> { ... }
    pub fn remove_placement_rule(&self, table: &str) -> ByteRagResult<bool> { ... }
}
```
...
**Files:**
- Modify: `core/byterag-core/src/replication/` — Shard 배치 시 PlacementRule 참조
...
```toml
# core/byterag-core/Cargo.toml (PITR 관련, 기존 의존성으로 충분)
# chrono는 이미 있음 (0.4)
