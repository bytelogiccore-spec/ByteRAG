    ...
    pub fn hash_join(
        ...
        #[cfg(feature = "gpu")]
        {
            ...
            unsafe { builder.launch(build_cfg) }
                .map_err(|e| ByteRagError::Gpu(format!("Build kernel launch failed: {:?}", e)))?;

            stream
                .synchronize()
                .map_err(|e| ByteRagError::Gpu(format!("Build stream sync failed: {:?}", e)))?;
            ...
            unsafe { builder.launch(probe_cfg) }
                .map_err(|e| ByteRagError::Gpu(format!("Probe kernel launch failed: {:?}", e)))?;

            stream
                .synchronize()
                .map_err(|e| ByteRagError::Gpu(format!("Probe stream sync failed: {:?}", e)))?;
