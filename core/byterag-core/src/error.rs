...
//!
//! All public APIs return `ByteRagResult<T>` — no panics in library code.

use thiserror::Error;

/// Unified error type for all DBX operations.
#[derive(Debug, Error)]
pub enum ByteRagError {
...
pub type ByteRagResult<T> = Result<T, ByteRagError>;

// From 구현들
impl From<serde_json::Error> for ByteRagError {
    fn from(err: serde_json::Error) -> Self {
        ByteRagError::Serialization(err.to_string())
    }
}
...
mod tests {
    ...
    fn error_display_storage() {
        let err = ByteRagError::Storage("disk full".to_string());
        assert_eq!(err.to_string(), "storage error: disk full");
    }

    #[test]
    fn error_display_table_not_found() {
        let err = ByteRagError::TableNotFound("users".to_string());
        assert_eq!(err.to_string(), "table 'users' not found");
    }

    #[test]
    fn error_display_type_mismatch() {
        let err = ByteRagError::TypeMismatch {
    ...
    fn byterag_result_ok() {
        let result: ByteRagResult<i32> = Ok(42);
        assert_eq!(result.unwrap(), 42);
    }

    #[test]
    fn byterag_result_err() {
        let result: ByteRagResult<i32> = Err(ByteRagError::KeyNotFound);
        assert!(result.is_err());
    }

    #[test]
    fn error_display_sql_parse() {
        let err = ByteRagError::SqlParse {
    ...
    fn error_display_sql_not_supported() {
        let err = ByteRagError::SqlNotSupported {
    ...
    fn error_display_transaction_conflict() {
        let err = ByteRagError::TransactionConflict {
    ...
    fn error_display_invalid_operation() {
        let err = ByteRagError::InvalidOperation {
