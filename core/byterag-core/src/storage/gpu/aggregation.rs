...
impl GpuManager {
    ...
    pub fn sum(&self, table: &str, column: &str) -> ByteRagResult<i64> {
        #[cfg(not(feature = "gpu"))]
        {
            let _ = (table, column);
            Err(ByteRagError::NotImplemented(
        ...
        #[cfg(feature = "gpu")]
        {
            ...
            match &*data {
                    ...
                    match strategy {
                        GpuReductionStrategy::Histogram => {
                            ...
                            let slice_host = stream.clone_dtoh(slice).map_err(|e| {
                                ByteRagError::Gpu(format!("Failed to copy slice to host: {:?}", e))
                            })?;
                            ...
                            let mut histogram_dev =
                                stream.alloc_zeros::<i64>(num_bins).map_err(|e| {
                                    ByteRagError::Gpu(format!("Failed to alloc histogram: {:?}", e))
                                })?;
                            ...
                            stream.synchronize().map_err(|e| {
                                ByteRagError::Gpu(format!("Stream sync failed: {:?}", e))
                            })?;

                            let histogram_host =
                                stream.clone_dtoh(&histogram_dev).map_err(|e| {
                                    ByteRagError::Gpu(format!("Failed to copy histogram: {:?}", e))
                                })?;
                        ...
                        GpuReductionStrategy::SinglePass => {
                            ...
                            let func = self.module.load_function("sum_i32").map_err(|_| {
                                ByteRagError::Gpu("Kernel sum_i32 not found".to_string())
                            })?;
                            ...
                            unsafe { builder.launch(cfg) }.map_err(|e| {
                                ByteRagError::Gpu(format!("Kernel launch failed: {:?}", e))
                            })?;
                        ...
                        GpuReductionStrategy::MultiPass => {
                            ...
                            let func_pass1 =
                                self.module.load_function("sum_i32_pass1").map_err(|_| {
                                    ByteRagError::Gpu("Kernel sum_i32_pass1 not found".to_string())
                                })?;
                            ...
                            builder.arg(&n);
                            unsafe { builder.launch(cfg) }.map_err(|e| {
                                ByteRagError::Gpu(format!("Pass1 kernel launch failed: {:?}", e))
                            })?;
                            ...
                            let func_pass2 =
                                self.module.load_function("sum_i32_pass2").map_err(|_| {
                                    ByteRagError::Gpu("Kernel sum_i32_pass2 not found".to_string())
                                })?;
                            ...
                            unsafe { builder2.launch(cfg_pass2) }.map_err(|e| {
                                ByteRagError::Gpu(format!("Pass2 kernel launch failed: {:?}", e))
                            })?;
                ...
                _ => Err(ByteRagError::NotImplemented(
    ...
    pub fn count(&self, table: &str, column: &str) -> ByteRagResult<u64> {
        #[cfg(not(feature = "gpu"))]
        {
            let _ = (table, column);
            Err(ByteRagError::NotImplemented(
        ...
        #[cfg(feature = "gpu")]
        {
            ...
            let func = self
                ...
                .map_err(|_| ByteRagError::Gpu("Kernel count_all not found".to_string()))?;
            ...
            unsafe { builder.launch(cfg) }
                .map_err(|e| ByteRagError::Gpu(format!("Kernel launch failed: {:?}", e)))?;
    ...
    pub fn min_max(&self, table: &str, column: &str, find_max: bool) -> ByteRagResult<i32> {
        #[cfg(not(feature = "gpu"))]
        {
    ...
    pub fn filter_gt(&self, table: &str, column: &str, threshold: i32) -> ByteRagResult<Vec<u8>> {
        #[cfg(not(feature = "gpu"))]
        {
            ...
            match &*data {
                GpuData::Int32(slice) => {
                    ...
                    let mask_host = stream
                        .clone_dtoh(&mask_dev)
                        .map_err(|e| ByteRagError::Gpu(format!("Failed to copy mask: {:?}", e)))?;
        ...
        match (delta_data, ros_data) {
            (GpuData::Int32(delta_slice), GpuData::Int32(ros_slice)) => {
                ...
                let func = self
                    ...
                    .map_err(|_| ByteRagError::Gpu("Kernel merge_sum_i32 not found".to_string()))?;
