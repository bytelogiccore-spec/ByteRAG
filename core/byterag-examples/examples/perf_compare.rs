...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tx.commit()?;
    let byterag_insert = insert_start.elapsed();
    ...
    let byterag_get = get_start.elapsed();

    let byterag_total = byterag_start.elapsed();

    println!(
        "  Insert {}K: {:?} ({:.0} ops/sec)",
        row_count / 1000,
        byterag_insert,
        row_count as f64 / byterag_insert.as_secs_f64()
    );
    println!(
        "  Get 1K:     {:?} ({:.0} ops/sec)",
        byterag_get,
        1000.0 / byterag_get.as_secs_f64()
    );
    println!("  Total:      {:?}\n", byterag_total);
    ...
    let insert_ratio = if byterag_insert < sqlite_insert {
        sqlite_insert.as_secs_f64() / byterag_insert.as_secs_f64()
    } else {
        byterag_insert.as_secs_f64() / sqlite_insert.as_secs_f64()
    };

    let get_ratio = if byterag_get < sqlite_get {
        sqlite_get.as_secs_f64() / byterag_get.as_secs_f64()
    } else {
