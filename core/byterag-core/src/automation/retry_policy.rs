...
impl RetryPolicy {
        ...
        F: FnMut() -> ByteRagResult<T>,
    {
        ...
        Err(last_err.unwrap_or_else(|| ByteRagError::InvalidArguments("no error recorded".into())))
    }
...
mod tests {
    ...
    fn test_retry_executes_until_success() {
        ...
        let result = policy.execute(move || {
            ...
            if count < 3 {
                Err(ByteRagError::Storage("fail".into()))
            } else {
                Ok(())
            }
        });
    ...
    fn test_retry_fails_after_max() {
        ...
        let result = policy.execute::<_, ()>(|| Err(ByteRagError::Storage("always fail".into())));
