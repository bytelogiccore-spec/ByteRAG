...
impl PhysicalPlanner {
    ...
    pub fn plan(&self, logical_plan: &LogicalPlan) -> ByteRagResult<PhysicalPlan> {
        match logical_plan {
            LogicalPlan::Scan {
                table,
                columns: _,
    ...
    fn plan_physical_expr(&self, expr: &Expr, schema: &[String]) -> ByteRagResult<PhysicalExpr> {
        match expr {
            Expr::Column(name) => {
                    ...
                    .position(|s| s.to_lowercase() == name.to_lowercase())
                {
                    ...
                    Err(ByteRagError::Schema(format!(
    ...
    fn extract_column_name(&self, expr: &Expr) -> ByteRagResult<String> {
        match expr {
            Expr::Column(name) => {
