...
/// # Example
/// ```
/// use byterag_core::automation::parse_create_schedule;
///
/// let sql = r#"
...
/// ```
pub fn parse_create_schedule(sql: &str) -> ByteRagResult<Schedule> {
    let sql = sql.trim();
...
/// # Example
/// ```
/// use byterag_core::automation::parse_drop_schedule;
///
/// let sql = "DROP SCHEDULE cleanup_job;";
/// let name = parse_drop_schedule(sql).unwrap();
/// assert_eq!(name, "cleanup_job");
/// ```
pub fn parse_drop_schedule(sql: &str) -> ByteRagResult<String> {
    let sql = sql.trim();
