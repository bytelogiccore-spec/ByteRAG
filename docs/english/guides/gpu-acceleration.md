...
```toml
[dependencies]
byterag-core = { version = "{{ site.byterag_version }}", features = ["gpu"] }
```
...
**Usage:**
```rust
use byterag_core::gpu::HashStrategy;
...
```rust
use byterag_core::storage::gpu::ShardingStrategy;
...
```rust
use byterag_core::storage::gpu::persistent::PersistentKernelManager;
...
```rust
use byterag_core::engine::stream::GpuStreamContext;
...
```rust
use byterag_core::gpu::GpuConfig;
...
fn benchmark_gpu_vs_cpu() -> byterag_core::ByteRagResult<()> {
    let db = Database::open_in_memory()?;
