...
//! ## 런타임으로 Transport 선택
//! ```rust,no_run
//! use byterag_core::replication::transport::{ReplicationConfig, TransportMode};
//! use byterag_core::engine::parallel_engine::DbConfig;
//!
//! // InMemory (기본 — 단일 프로세스)
...
pub mod quic {
    ...
    //! ## 사용법
    //! ```rust,ignore
    //! use byterag_core::replication::transport::quic::QuicNode;
    //! use std::path::Path;
    //!
    //! // 서버 (수신 노드)
