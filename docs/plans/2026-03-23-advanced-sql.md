...
**Files:**
- Modify: `core/byterag-core/src/sql/planner/types.rs` — `Expr::ScalarSubquery(Box<LogicalPlan>)` 추가
- Modify: `core/byterag-core/src/sql/planner/logical/` — 서브쿼리 Expression 변환
- Modify: `core/byterag-core/src/sql/executor/expr.rs` — 서브쿼리 평가
- Test: 통합 테스트
...
Run: `cargo test -p byterag-core test_scalar_subquery -v`
...
**Files:**
- Modify: `core/byterag-core/src/sql/planner/types.rs` — `Expr::InSubquery { expr, subquery, negated }` 추가
- Modify: `core/byterag-core/src/sql/executor/expr.rs` — IN 평가 (HashSet 기반)
- Test: 통합 테스트
...
**Files:**
- Modify: `core/byterag-core/src/sql/planner/types.rs` — `Expr::ExistsSubquery { subquery, negated }`
- Test: 통합 테스트
...
**Files:**
- Modify: `core/byterag-core/src/sql/planner/types.rs` — `LogicalPlan::CTE { name, body, query }` 추가
- Modify: `core/byterag-core/src/sql/planner/logical/` — `WITH` 절 변환
- Modify: `core/byterag-core/src/sql/interface.rs` — CTE 실행 플로우 추가
- Test: 통합 테스트
...
Run: `cargo test -p byterag-core test_non_recursive_cte -v`
Expected: FAIL
...
**Files:**
- Create: `core/byterag-core/src/sql/executor/operators/window.rs`
- Modify: `core/byterag-core/src/sql/executor/operators/mod.rs` — `pub mod window;`
- Modify: `core/byterag-core/src/sql/planner/types.rs` — `WindowExpr`, `WindowFrame` 등 타입 추가
- Modify: `core/byterag-core/src/sql/planner/physical.rs` — Window 물리 플랜 생성
- Test: 통합 테스트
...
impl PhysicalOperator for WindowOperator {
    fn execute(&self) -> ByteRagResult<Vec<RecordBatch>> {
...
**Files:**
- Modify: `core/byterag-core/src/sql/planner/types.rs` — `LogicalPlan::Union { plans, all }`
- Modify: `core/byterag-core/src/sql/interface.rs` — UNION 처리
- Test: 통합 테스트
...
**Files:**
- Create: `core/byterag-core/benches/advanced_sql_benchmark.rs`
- Modify: `core/byterag-core/Cargo.toml` — `[[bench]]` 추가
