...
impl MultiDeviceCoordinator {
    ...
    pub fn parallel_execute<F, R>(&self, f: F) -> ByteRagResult<Vec<R>>
    where
        F: Fn(usize, Arc<CudaContext>) -> ByteRagResult<R> + Send + Sync + 'static,
        R: Send + 'static,
    {
        use std::thread;
        ...
        for handle in handles {
            let result = handle
                .join()
                .map_err(|_| ByteRagError::Gpu("Thread join failed".to_string()))??;
            results.push(result);
        }
    ...
    pub fn synchronize_all(&self) -> ByteRagResult<()> {
        for device in &self.devices {
            device
                .synchronize()
                .map_err(|e| ByteRagError::Gpu(format!("Device sync failed: {:?}", e)))?;
        }
        Ok(())
    }
}
