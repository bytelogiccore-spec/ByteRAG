...
impl ScheduleExecutor {
    ...
    /// 스케줄 등록
    pub fn register(&self, schedule: Schedule) -> ByteRagResult<()> {
    ...
    /// 스케줄 활성화
    pub fn enable(&self, name: &str) -> ByteRagResult<()> {
        let mut schedules = self.schedules.write().unwrap();
    ...
    /// 스케줄 비활성화
    pub fn disable(&self, name: &str) -> ByteRagResult<()> {
        let mut schedules = self.schedules.write().unwrap();
        if let Some(schedule) = schedules.get_mut(name) {
            schedule.disable();
        }
    ...
    /// - SQL 실행 실패 시 에러 반환
    pub fn execute(&self, db: &crate::engine::Database, name: &str) -> ByteRagResult<()> {
        // 스케줄 조회
        let schedule = {
            let schedules = self.schedules.read().unwrap();
            schedules.get(name).cloned()
        ...
        let schedule = schedule.ok_or_else(|| crate::error::ByteRagError::InvalidOperation {
        ...
        for sql in &schedule.sql_body {
            match db.execute_sql(sql) {
                ...
                Err(e) => {
                    ...
                    return Err(crate::error::ByteRagError::InvalidOperation {
    ...
    pub fn start_scheduler(&self, db_weak: Weak<Database>) -> ByteRagResult<()> {
    ...
    /// 백그라운드 스케줄러 중지
    pub fn stop_scheduler(&self) -> ByteRagResult<()> {
        let mut thread = self.scheduler_thread.write().unwrap();
