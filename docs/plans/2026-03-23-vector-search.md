...
**Files:**
- Create: `core/byterag-core/src/vector/mod.rs`
- Create: `core/byterag-core/src/vector/types.rs`
- Modify: `core/byterag-core/src/lib.rs` — `pub mod vector;` 추가
- Test: `core/byterag-core/src/vector/types.rs` (inline tests)

**Step 1: Write the failing test**

```rust
// core/byterag-core/src/vector/types.rs
#[cfg(test)]
mod tests {
    use super::*;
...
Run: `cargo test -p byterag-core vector::types::tests --no-default-features`
...
```rust
// core/byterag-core/src/vector/mod.rs
pub mod types;
...
Run: `cargo test -p byterag-core vector::types::tests -v`
Expected: 5 tests PASS

**Step 5: Commit**

```bash
git add core/byterag-core/src/vector/ core/byterag-core/src/lib.rs
...
**Files:**
- Create: `core/byterag-core/src/vector/store.rs`
- Modify: `core/byterag-core/src/vector/mod.rs` — `pub mod store;` 추가
- Modify: `core/byterag-core/src/engine/database.rs` — vector store 필드 추가
- Test: `core/byterag-core/src/vector/store.rs` (inline tests)
...
Run: `cargo test -p byterag-core vector::store::tests --no-default-features`
Expected: FAIL — module `store` not found

**Step 3: Write minimal implementation**

```rust
// core/byterag-core/src/vector/store.rs
...
impl VectorStore {
    ...
    pub fn insert(&self, key: &[u8], data: Vec<f32>) -> ByteRagResult<()> {
        if data.len() != self.dimension {
            return Err(crate::error::ByteRagError::InvalidInput(
    ...
    pub fn get(&self, key: &[u8]) -> ByteRagResult<Option<VectorValue>> {
        Ok(self.vectors.get(key).map(|v| v.value().clone()))
    }

    pub fn delete(&self, key: &[u8]) -> ByteRagResult<bool> {
    ...
    pub fn search(&self, query: &[f32], k: usize) -> ByteRagResult<Vec<VectorSearchResult>> {
...
Run: `cargo test -p byterag-core vector::store::tests -v`
Expected: 3 tests PASS

**Step 5: Commit**

```bash
git add core/byterag-core/src/vector/store.rs core/byterag-core/src/vector/mod.rs
...
**Files:**
- Create: `core/byterag-core/src/engine/vector_api.rs`
- Modify: `core/byterag-core/src/engine/mod.rs` — `pub mod vector_api;` 추가
- Modify: `core/byterag-core/src/engine/database.rs` — `vector_stores: DashMap<String, VectorStore>` 필드 추가
- Test: `core/byterag-core/src/engine/vector_api.rs` (inline tests)
...
Run: `cargo test -p byterag-core engine::vector_api::tests --no-default-features`
...
- `create_vector_table(&self, name, dimension, metric) -> ByteRagResult<()>`
- `insert_vector(&self, table, key, data) -> ByteRagResult<()>`
- `get_vector(&self, table, key) -> ByteRagResult<Option<VectorValue>>`
- `delete_vector(&self, table, key) -> ByteRagResult<bool>`
- `vector_search(&self, table, query, k) -> ByteRagResult<Vec<VectorSearchResult>>`

**Step 4: Run test to verify it passes**

Run: `cargo test -p byterag-core engine::vector_api::tests -v`
Expected: 2 tests PASS

**Step 5: Commit**

```bash
git add core/byterag-core/src/engine/vector_api.rs core/byterag-core/src/engine/mod.rs core/byterag-core/src/engine/database.rs
...
**Files:**
- Create: `core/byterag-core/src/vector/hnsw.rs`
- Modify: `core/byterag-core/src/vector/mod.rs` — `pub mod hnsw;`
- Test: `core/byterag-core/src/vector/hnsw.rs` (inline tests)
...
**Files:**
- Modify: `core/byterag-core/src/vector/store.rs` — HNSW 인덱스 옵션 추가
- Test: `core/byterag-core/src/vector/store.rs` (기존 테스트 확장)
...
**Files:**
- Modify: `core/byterag-core/src/storage/kernels.cu` — 벡터 거리 커널 추가
- Create: `core/byterag-core/src/vector/gpu.rs` — CUDA 래퍼
- Modify: `core/byterag-core/src/vector/mod.rs` — `#[cfg(feature = "gpu")] pub mod gpu;`
...
**Files:**
- Modify: `core/byterag-core/src/sql/executor/expr.rs` — `vector_distance()` 함수 평가 추가
- Modify: `core/byterag-core/src/sql/planner/types.rs` — `Expr::VectorDistance` variant 추가
- Modify: `core/byterag-core/src/sql/interface.rs` — `CREATE VECTOR TABLE` DDL 처리
- Test: 별도 통합 테스트
...
**Files:**
- Create: `core/byterag-core/benches/vector_benchmark.rs`
- Modify: `core/byterag-core/Cargo.toml` — `[[bench]] name = "vector_benchmark"`
