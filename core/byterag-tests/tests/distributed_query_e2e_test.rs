...
use byterag_core::sql::executor::operators::{
    GridExchangeOperator, HashAggregateOperator, PhysicalOperator,
};
use byterag_core::sql::planner::types::{
...
fn test_fragment_splitter_produces_correct_plans() {
    use byterag_core::sql::executor::fragment_splitter::FragmentSplitter;
...
#[tokio::test]
async fn test_distributed_pipeline_in_process() -> ByteRagResult<()> {
    // ─── 데이터 셋업 ──────────────────────────────────────────
    ...
    let (merge_tx, merge_rx) = mpsc::channel::<ByteRagResult<Option<Vec<u8>>>>(32);
...
async fn test_distributed_group_by_sum_e2e_quic() -> ByteRagResult<()> {
