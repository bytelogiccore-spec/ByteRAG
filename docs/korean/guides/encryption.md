...
ByteRAG는 AES-256-GCM-SIV 및 ChaCha20-Poly1305 알고리즘을 사용하여 강력한 데이터 암호화를 지원합니다.
{: .fs-6 .fw-300 }
...
```rust
use byterag_core::storage::encryption::{EncryptionConfig, Algorithm};
...
암호화를 사용하면 약간의 성능 오버헤드가 발생하지만, ByteRAG의 병렬 처리 아키텍처를 통해 이를 최소화합니다.
