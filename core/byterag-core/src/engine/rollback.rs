...
impl Checkpoint {
    ...
    /// 상태 데이터 추가
    pub fn add_state<T: Serialize>(&mut self, key: String, value: &T) -> ByteRagResult<()> {
    ...
    /// 상태 데이터 조회
    pub fn get_state<T: for<'de> Deserialize<'de>>(&self, key: &str) -> ByteRagResult<T> {
        let json_value = self
            .state
            .get(key)
            .ok_or_else(|| ByteRagError::Serialization(format!("State key '{}' not found", key)))?;
...
impl RollbackManager {
    ...
    /// 체크포인트 생성
    pub fn create_checkpoint(&self, id: String, description: String) -> ByteRagResult<Checkpoint> {
    ...
    /// 체크포인트 저장
    fn save_checkpoint(&self, checkpoint: &Checkpoint) -> ByteRagResult<()> {
        // 디렉토리 생성
    ...
    /// 체크포인트 로드
    fn load_checkpoint(&self, id: &str) -> ByteRagResult<Checkpoint> {
        let file_path = self.checkpoint_dir.join(format!("{}.json", id));

        if !file_path.exists() {
            return Err(ByteRagError::Serialization(format!(
    ...
    /// 체크포인트로 롤백
    pub fn rollback_to_checkpoint(&self, id: &str) -> ByteRagResult<Checkpoint> {
        // 파일에서 로드
    ...
    /// 체크포인트 삭제
    pub fn delete_checkpoint(&self, id: &str) -> ByteRagResult<()> {
        // 메모리에서 삭제
    ...
    /// 자동 롤백 트리거
    pub fn trigger_auto_rollback(&self, reason: &str) -> ByteRagResult<()> {
