...
**Returns:**
- `ByteRagResult<Option<Vec<u8>>>`: Value if found, None if not found
...
fn main() {
    ...
    match db.insert("users", b"user:1", b"Alice") {
        ...
        Err(ByteRagError::DuplicateKey) => println!("Key already exists"),
...
fn insert_user(db: &Database, id: u32, name: &str) -> byterag_core::ByteRagResult<()> {
...
fn get_or_insert_default(
    ...
    key: &[u8], 
    default: &[u8]
) -> byterag_core::ByteRagResult<Vec<u8>> {
