...
LogCollector* log_collector_create(const char* db_path) {
    ...
    collector->db = byterag_open(db_path);
    
    byterag_execute_sql(collector->db,
...
void log_collector_add(LogCollector* collector, const char* level, const char* message) {
    ...
    byterag_execute_sql(collector->db, sql);
}

char* log_collector_get_errors(LogCollector* collector) {
    return byterag_execute_sql(collector->db, "SELECT * FROM logs WHERE level = 'ERROR'");
}

void log_collector_destroy(LogCollector* collector) {
    byterag_close(collector->db);
    free(collector);
}
...
int main() {
    DbxDatabase* db = byterag_open("sensors.db");
    ...
    // 통계 조회
    char* stats = byterag_execute_sql(db,
        "SELECT AVG(temperature), AVG(humidity) FROM sensor_data");
    printf("Stats: %s\n", stats);
    byterag_free_string(stats);
