...
impl SpillContext {
    ...
    pub fn new() -> ByteRagResult<Self> {
        Self::with_budget(128 * 1024 * 1024)
    }

    /// 지정된 메모리 예산(바이트)으로 SpillContext 생성.
    pub fn with_budget(budget_bytes: usize) -> ByteRagResult<Self> {
        let temp_dir = tempfile::tempdir()
            .map_err(|e| ByteRagError::Storage(format!("Spill: tempdir 생성 실패: {}", e)))?;
    ...
    pub fn spill_batches(&mut self, batches: &[RecordBatch]) -> ByteRagResult<PathBuf> {
        if batches.is_empty() {
            return Err(ByteRagError::Storage("Spill: 빈 batch 목록".to_string()));
        }
    ...
    fn write_ipc_file(&self, path: &PathBuf, batches: &[RecordBatch]) -> ByteRagResult<()> {
        let file = File::create(path).map_err(|e| {
            ByteRagError::Storage(format!("Spill: 파일 생성 실패 {}: {}", path.display(), e))
        })?;
        let writer_buf = BufWriter::new(file);
        let schema = batches[0].schema();

        let mut writer = FileWriter::try_new(writer_buf, &schema).map_err(|e| {
            ByteRagError::Storage(format!("Spill: Arrow IPC writer 초기화 실패: {}", e))
        })?;

        for batch in batches {
            writer
                .write(batch)
                .map_err(|e| ByteRagError::Storage(format!("Spill: batch 쓰기 실패: {}", e)))?;
        }

        writer
            .finish()
            .map_err(|e| ByteRagError::Storage(format!("Spill: IPC 파일 완료 실패: {}", e)))?;
    ...
    pub fn reload_batches(path: &PathBuf) -> ByteRagResult<Vec<RecordBatch>> {
        let file = File::open(path).map_err(|e| {
            ByteRagError::Storage(format!(
        ...
        let reader = FileReader::try_new(file, None).map_err(|e| {
            ByteRagError::Storage(format!("Spill: Arrow IPC reader 초기화 실패: {}", e))
        })?;
