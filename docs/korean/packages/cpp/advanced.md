...
if (byterag_insert(db, "users", (uint8_t*)"user:1", 6, (uint8_t*)"Alice", 5) != 0) {
    byterag_rollback(tx);
    return 1;
}
...
if (byterag_execute_sql(db, sql) != 0) {
...
// C
byterag_execute_sql(db, "DROP TRIGGER audit_trigger;");
```
...
// C
byterag_execute_sql(db, "CALL update_balance(123, 100.50);");
byterag_execute_sql(db, "CALL cleanup_old_data(30);");
```
...
// C
byterag_execute_sql(db, "DROP PROCEDURE update_balance;");
```
...
// C
byterag_execute_sql(db, "DROP SCHEDULE refresh_stats;");
```
...
void* worker(void* arg) {
    ...
    DbxDatabase* db = byterag_open(data->db_path);
...
```c
byterag_enable_feature(db, "parallel_query");
byterag_enable_feature(db, "query_plan_cache");
byterag_disable_feature(db, "parallel_query");

if (byterag_is_feature_enabled(db, "parallel_query")) {
    printf("병렬 쿼리 활성화됨\n");
}
```
