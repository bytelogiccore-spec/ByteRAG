...
/// # 예시
/// ```rust
/// use byterag_core::engine::parallel_engine::ParallelismConfig;
...
impl ParallelExecutionEngine {
    ...
    pub fn new(policy: ParallelizationPolicy) -> ByteRagResult<Self> {
    ...
    pub fn new_with_config(policy: ParallelizationPolicy, config: DbConfig) -> ByteRagResult<Self> {
        ...
        let thread_pool = ThreadPoolBuilder::new()
            ...
            .build()
            .map_err(|e| {
                ByteRagError::NotImplemented(format!("Failed to create thread pool: {}", e))
            })?;
...
/// # 예시
/// ```rust
/// use byterag_core::engine::parallel_engine::{DbConfig, ParallelismConfig, DirtyBufferMode};
/// let config = DbConfig {
