...
impl GpuStreamContext {
    pub fn new(
        ...
        let stream = device
            .fork_default_stream()
            .map_err(|e| ByteRagError::Gpu(format!("Failed to create stream: {:?}", e)))?;
    ...
    pub fn synchronize(&self) -> ByteRagResult<()> {
        self.stream
            .synchronize()
...
impl StreamManager {
    ...
    pub fn new(device: Arc<CudaContext>) -> ByteRagResult<Self> {
        Ok(Self {
            device,
            streams: Vec::new(),
            next_id: 0,
    ...
    pub fn create_stream(&mut self, priority: StreamPriority) -> ByteRagResult<usize> {
    ...
    pub fn synchronize_all(&self) -> ByteRagResult<()> {
...
#[cfg(not(feature = "gpu"))]
impl StreamManager {
    pub fn new(_device: ()) -> ByteRagResult<Self> {
