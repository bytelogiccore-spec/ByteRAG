...
impl TriggerRegistry {
    ...
    pub fn register(&self, hook: Arc<EventHook>) -> ByteRagResult<()> {
        let mut triggers = self
            .triggers
            .write()
            .map_err(|_| crate::error::ByteRagError::LockPoisoned)?;

        // 중복 이름 체크
        if triggers.iter().any(|t| t.name() == hook.name()) {
            return Err(crate::error::ByteRagError::DuplicateCallable(
                hook.name().to_string(),
            ));
        }
    ...
    pub fn unregister(&self, name: &str) -> ByteRagResult<()> {
        ...
        let pos = triggers
            ...
            .ok_or_else(|| crate::error::ByteRagError::CallableNotFound(name.to_string()))?;
...
impl Database {
    ...
    /// use byterag_core::automation::{EventHook, EventHookEventType, EventHookCondition, EventHookAction};
    ///
    ...
    /// # }
    /// ```
    pub fn register_trigger(&self, hook: EventHook) -> ByteRagResult<()> {
    ...
    /// 트리거 등록 해제
    pub fn unregister_trigger(&self, name: &str) -> ByteRagResult<()> {
    ...
    pub fn fire_trigger(&self, event: EventHookEvent) -> ByteRagResult<Vec<String>> {
    ...
    /// 등록된 트리거 목록
    pub fn list_triggers(&self) -> ByteRagResult<Vec<String>> {
