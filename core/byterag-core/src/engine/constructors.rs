...
impl Database {
    ...
    pub fn open(path: &Path) -> ByteRagResult<Arc<Self>> {
        ...
        let mut db = Self {
            ...
            metrics: Arc::new(crate::monitoring::ByteRagMetrics::new()),
        ...
        let apply_fn = |record: &crate::wal::WalRecord| -> ByteRagResult<()> {
            match record {
    ...
    pub fn open_encrypted(path: &Path, encryption: EncryptionConfig) -> ByteRagResult<Self> {
    ...
    /// # }
    /// ```
    pub fn open_in_memory_encrypted(encryption: EncryptionConfig) -> ByteRagResult<Self> {
    ...
    ///
    /// * `path` - 데이터베이스 파일 경로
    pub fn open_safe(path: impl AsRef<Path>) -> ByteRagResult<Arc<Self>> {
    ...
    /// * `path` - 데이터베이스 파일 경로
    ///
    pub fn open_fast(path: impl AsRef<Path>) -> ByteRagResult<Arc<Self>> {
...
impl Database {
    ...
    /// ```rust,no_run
    /// use byterag_core::Database;
    /// use byterag_core::engine::parallel_engine::{DbConfig, ParallelismConfig};
    /// use std::path::Path;
    ///
    ...
    pub fn open_with_config(
        path: &std::path::Path,
        config: DbConfig,
    ) -> ByteRagResult<std::sync::Arc<Self>> {
