...
```rust
use byterag_core::engine::feature_flags::{FeatureFlags, Feature};
...
```rust
// JSON 파일로 저장
flags.save_to_file("./byterag_features.json")?;

// 파일에서 로드
let flags = FeatureFlags::load_from_file("./byterag_features.json")?;
```
...
```bash
# 환경변수 설정
export byterag_FEATURE_PARALLEL_QUERY=true
export byterag_FEATURE_QUERY_PLAN_CACHE=true
```
