...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  >> Inserted & Retrieved: {:?}", String::from_utf8(val.unwrap()));
    ...
    db.insert("doc_embeddings", b"doc:rust_memory_safety", as_bytes(&doc1_vec))?;
    db.insert("doc_embeddings", b"doc:python_async", as_bytes(&doc2_vec))?;
    db.insert("doc_embeddings", b"doc:rust_concurrency", as_bytes(&doc3_vec))?;
    ...
    for (i, res) in vec_results.iter().enumerate() {
        println!("  >> Top {}: ID = {}, Score = {:.4}", 
            i + 1, String::from_utf8_lossy(&res.id), res.score);
    }
    ...
    db.insert("call_graph", b"open_storage_engine()", b"load_arrow_cache()")?;
