...
impl GpuManager {
    ...
    pub fn upload_batch(&self, table: &str, batch: &RecordBatch) -> ByteRagResult<()> {
        #[cfg(not(feature = "gpu"))]
        {
            let _ = (table, batch);
    ...
    #[cfg(feature = "gpu")]
    fn convert_and_upload(&self, array: &Arc<dyn Array>) -> ByteRagResult<GpuData> {
    ...
    pub fn upload_batch_pinned(&self, table: &str, batch: &RecordBatch) -> ByteRagResult<()> {
        #[cfg(not(feature = "gpu"))]
        {
            let _ = (table, batch);
            Err(ByteRagError::NotImplemented(
            ...
            for (i, column) in batch.columns().iter().enumerate() {
                ...
                if column.data_type() == &arrow::datatypes::DataType::Int32 {
                    ...
                    let mut pinned = unsafe { self.device.alloc_pinned::<i32>(values.len()) }
                        .map_err(|e| {
                            ByteRagError::Gpu(format!("Failed to alloc pinned memory: {:?}", e))
                        })?;
                    ...
                    let slice = stream.clone_htod(&pinned).map_err(|e| {
                        ByteRagError::Gpu(format!("CUDA pinned HTOD copy failed: {:?}", e))
                    })?;
