...
impl TableStore {
    pub fn open(path: impl AsRef<Path>) -> ByteRagResult<Self> {
        Self::open_with_mode(path, DirtyBufferMode::default())
    }

    /// dirty 버퍼 모드를 지정하여 열기.
    pub fn open_with_mode(path: impl AsRef<Path>, mode: DirtyBufferMode) -> ByteRagResult<Self> {
    ...
    fn load_index(&mut self) -> ByteRagResult<()> {
    ...
    fn replay_wal_file(&mut self) -> ByteRagResult<()> {
    ...
    fn read_index_offset(&mut self) -> ByteRagResult<u64> {
        self.file.seek(SeekFrom::End(-(FOOTER_SIZE as i64)))?;
        let mut footer = [0u8; 16];
        self.file.read_exact(&mut footer)?;
    ...
    fn read_page_at(&mut self, page_idx: usize) -> ByteRagResult<WosPage> {
        if page_idx >= self.page_index.len() {
            return Err(ByteRagError::Storage("page index out of range".into()));
        }
        // LRU 캐시 조회
    ...
    fn compact(&mut self) -> ByteRagResult<()> {
        ...
        if self.has_flushed_data {
            ...
            let pages: Vec<ByteRagResult<WosPage>> = if page_bufs.len() >= PARALLEL_PAGE_THRESHOLD {
                use rayon::prelude::*;
                page_bufs
                    .par_iter()
    ...
    fn write_sstable(&mut self, all: BTreeMap<Vec<u8>, Vec<u8>>) -> ByteRagResult<()> {
    ...
    pub fn insert(&mut self, key: &[u8], value: &[u8]) -> ByteRagResult<()> {
        self.dirty
            .insert_entry(key.to_vec(), DirtyState::Put(value.to_vec()));
        Ok(())
    }

    pub fn get(&mut self, key: &[u8]) -> ByteRagResult<Option<Vec<u8>>> {
        // 1. dirty (가장 최신)
    ...
    pub fn delete(&mut self, key: &[u8]) -> ByteRagResult<bool> {
