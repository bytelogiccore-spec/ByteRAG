...
//!
//! ```rust
//! use byterag_core::wal::WriteAheadLog;
//! use byterag_core::wal::checkpoint::CheckpointManager;
//! use std::sync::Arc;
//! use std::path::Path;
//!
...
impl CheckpointManager {
    ...
    /// let checkpoint_mgr = CheckpointManager::new(wal, Path::new("./wal.log"));
    ///
    /// let apply_fn = |record: &WalRecord| -> byterag_core::ByteRagResult<()> {
    ...
    /// # }
    /// ```
    pub fn checkpoint<F>(&self, apply_fn: F) -> ByteRagResult<u64>
    where
        F: Fn(&WalRecord) -> ByteRagResult<()>,
    {
    ...
    /// # }
    /// ```
    pub fn recover<F>(wal_path: &Path, apply_fn: F) -> ByteRagResult<usize>
    where
        F: Fn(&WalRecord) -> ByteRagResult<()>,
    {
    ...
    /// # }
    /// ```
    pub fn trim_before(&self, sequence: u64) -> ByteRagResult<()> {
        // Read all records
        let records = self.wal.replay()?;
