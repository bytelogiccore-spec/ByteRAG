...
/// END;
/// ```
pub fn parse_create_procedure(sql: &str) -> ByteRagResult<StoredProcedure> {
    let sql = sql.trim();
...
/// ```
pub fn parse_drop_procedure(sql: &str) -> ByteRagResult<String> {
    let sql = sql.trim();
...
/// ```
pub fn parse_call_procedure(sql: &str) -> ByteRagResult<(String, Vec<String>)> {
    let sql = sql.trim();
