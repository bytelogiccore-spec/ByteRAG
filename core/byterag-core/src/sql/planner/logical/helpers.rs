...
pub fn convert_binary_op(op: &SqlBinaryOp) -> ByteRagResult<BinaryOperator> {
    match op {
...
pub fn match_scalar_function(name: &str) -> Option<ScalarFunction> {
    match name {
        ...
        // 벡터 및 그래프 연산
        "VECTOR_COSINE" | "COSINE_SIMILARITY" => Some(ScalarFunction::VectorCosine),
        "VECTOR_L2" | "L2_DISTANCE" => Some(ScalarFunction::VectorL2),
        "GRAPH_NEIGHBORS" | "GRAPH_TRAVERSE" => Some(ScalarFunction::GraphNeighbors),
...
pub fn extract_usize(expr: &SqlExpr) -> ByteRagResult<usize> {
    match expr {
        SqlExpr::Value(sqlparser::ast::Value::Number(n, _)) => n.parse::<usize>().map_err(|_| {
            ByteRagError::Schema(format!(
