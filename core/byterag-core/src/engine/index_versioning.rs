...
impl IndexVersionManager {
    ...
    pub fn create_index(
        ...
        let mut versions = self
            .versions
            .write()
            .map_err(|_| ByteRagError::Serialization("Lock poisoned".into()))?;
        let mut active = self
            .active
            .write()
            .map_err(|_| ByteRagError::Serialization("Lock poisoned".into()))?;
    ...
    pub fn start_reindex(
        ...
        let history = versions
            .get_mut(name)
            .ok_or_else(|| ByteRagError::Serialization(format!("Index {name} not found")))?;

        let last = history
            .last()
            .ok_or_else(|| ByteRagError::Serialization("Empty history".into()))?;
    ...
    pub fn complete_reindex(&self, name: &str, version: u64) -> ByteRagResult<()> {
        let mut versions = self
            .versions
            .write()
            .map_err(|_| ByteRagError::Serialization("Lock poisoned".into()))?;
    ...
    /// 현재 활성 인덱스 메타데이터 조회
    pub fn get_active(&self, name: &str) -> ByteRagResult<IndexMeta> {
        let versions = self
            .versions
            .read()
            .map_err(|_| ByteRagError::Serialization("Lock poisoned".into()))?;
        ...
        history
            ...
            .cloned()
            .ok_or_else(|| ByteRagError::Serialization(format!("Version {active_ver} not found")))
    }

    /// 인덱스 삭제
    pub fn drop_index(&self, name: &str) -> ByteRagResult<()> {
        let mut versions = self
            .versions
            .write()
            .map_err(|_| ByteRagError::Serialization("Lock poisoned".into()))?;
    ...
    /// 테이블의 모든 인덱스 조회
    pub fn list_indexes(&self, table: &str) -> ByteRagResult<Vec<IndexMeta>> {
        let versions = self
            .versions
            .read()
            .map_err(|_| ByteRagError::Serialization("Lock poisoned".into()))?;
