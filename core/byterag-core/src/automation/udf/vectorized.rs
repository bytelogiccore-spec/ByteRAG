...
type VectorizedFn = Box<dyn Fn(&[Value]) -> ByteRagResult<Value> + Send + Sync>;
...
impl VectorizedUDF {
    ...
    where
        F: Fn(&[Value]) -> ByteRagResult<Value> + Send + Sync + 'static,
    {
        Self {
            name: name.into(),
            signature,
...
impl Callable for VectorizedUDF {
    fn call(&self, _ctx: &ExecutionContext, args: &[Value]) -> ByteRagResult<Value> {
...
mod tests {
    ...
    fn test_vectorized_udf_mismatch_lengths() {
        ...
        match result.unwrap_err() {
            ByteRagError::InvalidArguments(msg) => assert!(msg.contains("same array length")),
