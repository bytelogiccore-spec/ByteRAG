...
impl QuicChannel {
    ...
    pub async fn new(
        ...
        let tls_builder = s2n_quic::provider::tls::default::Server::builder()
            ...
            .map_err(|e| crate::error::ByteRagError::Network(e.to_string()))?;
        let tls = tls_builder
            .build()
            .map_err(|e| crate::error::ByteRagError::Network(e.to_string()))?;
    ...
    pub async fn send_message(
        ...
        let mut connection = match self.client.connect(connect_config).await {
            Ok(c) => c,
            Err(e) => return Err(crate::error::ByteRagError::Network(e.to_string())),
        };
        ...
        if let Err(e) = stream.write_all(&len).await {
            return Err(crate::error::ByteRagError::Network(e.to_string()));
        }
    ...
    pub async fn send_request_and_wait(
        ...
        peer_addr: SocketAddr,
        msg: crate::grid::protocol::GridMessage,
    ) -> ByteRagResult<crate::grid::protocol::GridMessage> {
        let connect_config =
            s2n_quic::client::Connect::new(peer_addr).with_server_name("localhost");
        let mut connection = match self.client.connect(connect_config).await {
            Ok(c) => c,
...
impl GridMessageWrapper {
    ...
    pub async fn send_reply(&mut self, reply: crate::grid::protocol::GridMessage) -> ByteRagResult<()> {
        if let Some(stream) = &mut self.stream {
            ...
            Err(crate::error::ByteRagError::Network(
