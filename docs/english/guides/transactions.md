...
fn main() -> byterag_core::ByteRagResult<()> {
    ...
    match tx2.commit() {
        Err(ByteRagError::WriteConflict) => {
...
fn update_with_optimistic_lock(
    ...
    let current = match tx.get("documents", key)? {
        ...
        None => return Err(byterag_core::ByteRagError::NotFound),
    };
...
fn retry_on_conflict<F>(db: &Database, mut f: F) -> byterag_core::ByteRagResult<()>
where
    F: FnMut(&byterag_core::Transaction) -> byterag_core::ByteRagResult<()>,
{
    ...
    for attempt in 0..MAX_RETRIES {
        ...
        match f(&tx) {
            Ok(_) => {
                match tx.commit() {
                    Ok(_) => return Ok(()),
                    Err(ByteRagError::WriteConflict) if attempt < MAX_RETRIES - 1 => {
                        // Retry
                        continue;
                    }
                    Err(e) => return Err(e),
...
fn increment_counter(db: &Database, key: &[u8]) -> byterag_core::ByteRagResult<u64> {
    let tx = db.begin_transaction()?;
...
fn main() -> byterag_core::ByteRagResult<()> {
    ...
    match tx.commit() {
        ...
        Err(ByteRagError::TransactionAborted) => {
