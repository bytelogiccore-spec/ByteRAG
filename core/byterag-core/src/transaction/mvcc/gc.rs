...
//!
//! ```rust
//! use byterag_core::transaction::gc::GarbageCollector;
...
impl GarbageCollector {
    ...
    pub fn collect(&self, db: &Database, min_active_ts: u64) -> ByteRagResult<usize> {
    ...
    pub fn estimate_garbage(&self, db: &Database, min_active_ts: u64) -> ByteRagResult<usize> {
...
mod tests {
    ...
    #[test]
    fn test_gc_removes_old_versions() -> ByteRagResult<()> {
        let db = Database::open_in_memory()?;
        let gc = GarbageCollector::new();
    ...
    #[test]
    fn test_gc_keeps_minimum_versions() -> ByteRagResult<()> {
        let db = Database::open_in_memory()?;
    ...
    #[test]
    fn test_gc_multiple_keys() -> ByteRagResult<()> {
        let db = Database::open_in_memory()?;
        let gc = GarbageCollector::new();
    ...
    #[test]
    fn test_gc_estimate_accuracy() -> ByteRagResult<()> {
        let db = Database::open_in_memory()?;
        let gc = GarbageCollector::new();
    ...
    #[test]
    fn test_gc_empty_database() -> ByteRagResult<()> {
        let db = Database::open_in_memory()?;
        let gc = GarbageCollector::new();
