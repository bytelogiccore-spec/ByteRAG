...
impl ErasureCodingStore {
    ...
    /// 개별 샤드를 디스크에 저장
    pub fn store_shard(&self, key: &str, shard_id: usize, data: &[u8]) -> ByteRagResult<()> {
    ...
    /// 샤드를 디스크에서 읽기
    pub fn fetch_shard(&self, key: &str, shard_id: usize) -> ByteRagResult<Option<Vec<u8>>> {
        let shard_path = self
            .base_dir
            .join(key)
    ...
    pub fn encode_and_store(&self, key: &str, data: &[u8]) -> ByteRagResult<()> {
    ...
    pub fn retrieve_and_decode(&self, key: &str) -> ByteRagResult<Option<Vec<u8>>> {
        let object_dir = self.base_dir.join(key);
    ...
    /// Delete EC fragments
    pub fn delete(&self, key: &str) -> ByteRagResult<bool> {
        let object_dir = self.base_dir.join(key);
        if object_dir.exists() {
