...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  >> INSERT 10K completed in: {:.2?} ({:.0} ops/sec)",
        insert_dur, insert_ops
    );
    ...
    println!(
        "  >> GET 10K completed in: {:.2?} ({:.0} ops/sec, hits: {})",
        get_dur, get_ops, hit_count
    );
    ...
    println!(
        "  >> SCAN 10K completed in: {:.2?} (scanned {} items)",
        scan_dur,
        scan_results.len()
    );
    ...
    println!(
        "   - INSERT: {:.3} ms ({:.1} ns/op)",
        insert_dur.as_secs_f64() * 1000.0,
        insert_dur.as_nanos() as f64 / count as f64
    );
    println!(
        "   - GET:    {:.3} ms ({:.1} ns/op)",
        get_dur.as_secs_f64() * 1000.0,
        get_dur.as_nanos() as f64 / count as f64
    );
