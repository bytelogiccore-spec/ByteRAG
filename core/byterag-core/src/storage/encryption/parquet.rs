...
impl EncryptedParquetReader {
    ...
    pub fn read(path: &Path, encryption: &EncryptionConfig) -> ByteRagResult<Vec<RecordBatch>> {
        // Step 1: Read file
        ...
        if data[4] != VERSION {
            return Err(ByteRagError::Encryption(format!(
    ...
    pub fn is_encrypted_parquet(path: &Path) -> ByteRagResult<bool> {
        let mut file = File::open(path)?;
