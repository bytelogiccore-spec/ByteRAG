...
use byterag_core::engine::{BenchmarkRunner, TwoLevelCache};
...
#[test]
fn test_regression_two_level_cache_l1_get() -> ByteRagResult<()> {
...
#[test]
fn test_regression_parallel_sql_parser_batch_100() -> ByteRagResult<()> {
    let runner = BenchmarkRunner::new()
        .with_baseline_path(PathBuf::from("target/phase1_baseline.json"))
        .with_threshold(1.2); // 20% 성능 저하 허용
...
#[test]
fn test_regression_version_manager_get_at_snapshot() -> ByteRagResult<()> {
    let runner = BenchmarkRunner::new()
        .with_baseline_path(PathBuf::from("target/phase1_baseline.json"))
        .with_threshold(1.2); // 20% 성능 저하 허용
