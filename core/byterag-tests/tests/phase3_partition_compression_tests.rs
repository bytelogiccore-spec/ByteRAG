...
use byterag_core::storage::compression::{CompressionAlgorithm, CompressionConfig};
use byterag_core::storage::partition::{PartitionMap, PartitionType};

#[test]
fn test_set_and_get_partition_compression() -> ByteRagResult<()> {
...
#[test]
fn test_partition_compression_default_fallback() -> ByteRagResult<()> {
    let db = Database::open_in_memory()?;
...
#[test]
fn test_partition_compression_lz4() -> ByteRagResult<()> {
    let db = Database::open_in_memory()?;
...
#[test]
fn test_partition_compression_overwrite() -> ByteRagResult<()> {
    let db = Database::open_in_memory()?;
