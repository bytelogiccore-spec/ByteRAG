...
impl GpuManager {
    ...
    pub(super) fn radix_sort_i32_i64(
    ...
    ) -> ByteRagResult<(CudaSlice<i32>, CudaSlice<i64>)> {
        let n = keys.len();
        ...
        let mut keys_buf2 = stream
            .alloc_zeros::<i32>(n)
            .map_err(|e| ByteRagError::Gpu(format!("Failed to alloc keys buffer: {:?}", e)))?;

        let mut values_buf1 = values.clone();
        let mut values_buf2 = stream
            .alloc_zeros::<i64>(n)
            .map_err(|e| ByteRagError::Gpu(format!("Failed to alloc values buffer: {:?}", e)))?;
        ...
        for pass in 0..4 {
            ...
            let mut temp_histogram = stream
                .alloc_zeros::<i32>(256)
                .map_err(|e| ByteRagError::Gpu(format!("Failed to alloc histogram: {:?}", e)))?;
            let mut temp_prefix = stream
                .alloc_zeros::<i32>(256)
                .map_err(|e| ByteRagError::Gpu(format!("Failed to alloc prefix: {:?}", e)))?;
    ...
    pub(super) fn group_by_sum_sorted(
        ...
        let mut out_keys = stream
            .alloc_zeros::<i32>(n)
            .map_err(|e| ByteRagError::Gpu(format!("Failed to alloc out_keys: {:?}", e)))?;
        let mut out_sums = stream
            .alloc_zeros::<i64>(n)
            .map_err(|e| ByteRagError::Gpu(format!("Failed to alloc out_sums: {:?}", e)))?;
        let mut out_counts = stream
            .alloc_zeros::<i32>(n)
            .map_err(|e| ByteRagError::Gpu(format!("Failed to alloc out_counts: {:?}", e)))?;
        let mut num_groups_dev = stream
            .alloc_zeros::<i32>(1)
            .map_err(|e| ByteRagError::Gpu(format!("Failed to alloc num_groups: {:?}", e)))?;
        ...
        unsafe { builder.launch(cfg) }
            .map_err(|e| ByteRagError::Gpu(format!("Sorted aggregation failed: {:?}", e)))?;
        ...
        let num_groups_vec = stream
            ...
            .map_err(|e| ByteRagError::Gpu(format!("Failed to copy num_groups: {:?}", e)))?;
        let num_groups = num_groups_vec[0] as usize;

        let keys_vec = stream
            .clone_dtoh(&out_keys)
            .map_err(|e| ByteRagError::Gpu(format!("Failed to copy keys: {:?}", e)))?;
        let sums_vec = stream
            .clone_dtoh(&out_sums)
            .map_err(|e| ByteRagError::Gpu(format!("Failed to copy sums: {:?}", e)))?;
        let counts_vec = stream
            .clone_dtoh(&out_counts)
            .map_err(|e| ByteRagError::Gpu(format!("Failed to copy counts: {:?}", e)))?;
