...
///
/// ```rust
/// use byterag_core::transaction::version_manager::VersionManager;
/// use byterag_core::transaction::versionable::Versionable;
/// use byterag_core::transaction::manager::TimestampOracle;
/// use std::sync::Arc;
///
...
impl<T: Versionable> VersionManager<T> {
    ...
    ///
    /// 성공 시 `Ok(())`, 실패 시 에러
    pub fn add_version(&self, key: Vec<u8>, value: T, commit_ts: u64) -> ByteRagResult<()> {
        let mut versions = self
            .versions
            .write()
            .map_err(|e| ByteRagError::Storage(format!("Lock error: {}", e)))?;
    ...
    /// - `Err(_)` - 에러 발생
    pub fn get_at_snapshot(&self, key: &[u8], read_ts: u64) -> ByteRagResult<Option<T>> {
        let versions = self
            .versions
            .read()
            .map_err(|e| ByteRagError::Storage(format!("Lock error: {}", e)))?;
    ...
    ///
    /// 삭제된 버전의 개수
    pub fn collect_garbage(&self, min_active_ts: u64) -> ByteRagResult<usize> {
        let mut versions = self
            .versions
            .write()
            .map_err(|e| ByteRagError::Storage(format!("Lock error: {}", e)))?;
    ...
    pub fn version_count(&self, key: &[u8]) -> ByteRagResult<usize> {
        let versions = self
            .versions
            .read()
            .map_err(|e| ByteRagError::Storage(format!("Lock error: {}", e)))?;
    ...
    /// 전체 키 개수를 반환합니다.
    pub fn key_count(&self) -> ByteRagResult<usize> {
        let versions = self
            .versions
            .read()
            .map_err(|e| ByteRagError::Storage(format!("Lock error: {}", e)))?;
    ...
    /// 전체 버전 개수를 반환합니다.
    pub fn total_version_count(&self) -> ByteRagResult<usize> {
        let versions = self
            .versions
            .read()
            .map_err(|e| ByteRagError::Storage(format!("Lock error: {}", e)))?;
...
mod tests {
    ...
    #[test]
    fn test_version_manager_add_and_get() -> ByteRagResult<()> {
        let oracle = Arc::new(TimestampOracle::default());
        let manager = VersionManager::<String>::new(Arc::clone(&oracle));
    ...
    #[test]
    fn test_version_manager_snapshot_isolation() -> ByteRagResult<()> {
        let oracle = Arc::new(TimestampOracle::default());
    ...
    #[test]
    fn test_version_manager_garbage_collection() -> ByteRagResult<()> {
        let oracle = Arc::new(TimestampOracle::default());
        let manager = VersionManager::<String>::new(Arc::clone(&oracle));
    ...
    #[test]
    fn test_version_manager_multiple_keys() -> ByteRagResult<()> {
    ...
    #[test]
    fn test_version_manager_gc_preserves_latest() -> ByteRagResult<()> {
