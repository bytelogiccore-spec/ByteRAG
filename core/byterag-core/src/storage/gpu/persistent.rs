...
impl PersistentKernelManager {
    ...
    pub fn compile_kernel(&mut self) -> ByteRagResult<()> {
        use cudarc::nvrtc::Ptx;

        let ptx = Ptx::compile_source(PERSISTENT_KERNEL_SRC)
            .map_err(|e| ByteRagError::Gpu(format!("NVRTC compilation failed: {:?}", e)))?;

        let module = self
            .device
            .load_module(ptx)
            .map_err(|e| ByteRagError::Gpu(format!("Module load failed: {:?}", e)))?;
    ...
    pub fn get_kernel_function(&self) -> ByteRagResult<Option<Arc<cudarc::driver::CudaFunction>>> {
        match &self.module {
            Some(module) => {
                let func = module
                    .load_function("persistent_scan_kernel")
                    .map_err(|e| {
                        ByteRagError::Gpu(format!("Failed to load kernel function: {:?}", e))
                    })?;
                Ok(Some(func))
            }
            None => Ok(None),
