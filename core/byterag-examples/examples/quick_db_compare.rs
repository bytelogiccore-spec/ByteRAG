...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    db.flush()?;
    let byterag_insert = start.elapsed();
    ...
    let byterag_get = start.elapsed();

    println!(
        "  Insert: {:?} ({:.0} ops/sec)",
        byterag_insert,
        TEST_SIZE as f64 / byterag_insert.as_secs_f64()
    );
    println!(
        "  Get:    {:?} ({:.0} ops/sec)\n",
        byterag_get,
        TEST_SIZE as f64 / byterag_get.as_secs_f64()
    );
    ...
    println!(
        "    SQLite3: {:.2}x {}",
        byterag_insert.as_secs_f64() / sqlite_insert.as_secs_f64(),
        if byterag_insert < sqlite_insert {
            "faster"
        } else {
            "slower"
        }
    ...
    println!(
        "    Sled:    {:.2}x {}",
        byterag_insert.as_secs_f64() / sled_insert.as_secs_f64(),
        if byterag_insert < sled_insert {
            "faster"
        } else {
            "slower"
        }
    ...
    println!(
        "    Redb:    {:.2}x {}",
        byterag_insert.as_secs_f64() / redb_insert.as_secs_f64(),
        if byterag_insert < redb_insert {
            "faster"
        } else {
            "slower"
        }
    ...
    println!(
        "    SQLite3: {:.2}x {}",
        byterag_get.as_secs_f64() / sqlite_get.as_secs_f64(),
        if byterag_get < sqlite_get {
            "faster"
        } else {
            "slower"
        }
    ...
    println!(
        "    Sled:    {:.2}x {}",
        byterag_get.as_secs_f64() / sled_get.as_secs_f64(),
        if byterag_get < sled_get {
            "faster"
        } else {
            "slower"
        }
    ...
    println!(
        "    Redb:    {:.2}x {}",
        byterag_get.as_secs_f64() / redb_get.as_secs_f64(),
        if byterag_get < redb_get {
            "faster"
        } else {
            "slower"
        }
