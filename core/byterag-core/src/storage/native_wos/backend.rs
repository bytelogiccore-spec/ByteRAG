...
impl NativeWosBackend {
    pub fn open(base_path: &Path) -> ByteRagResult<Self> {
        Self::open_with_mode(base_path, DirtyBufferMode::default())
    }

    /// dirty 버퍼 모드를 지정하여 열기.
    pub fn open_with_mode(base_path: &Path, mode: DirtyBufferMode) -> ByteRagResult<Self> {
    ...
    pub fn open_temporary() -> ByteRagResult<Self> {
        Self::open_temporary_with_mode(DirtyBufferMode::default())
    }

    /// 임시 백엔드 + dirty 모드 지정
    pub fn open_temporary_with_mode(mode: DirtyBufferMode) -> ByteRagResult<Self> {
    ...
    fn get_or_open(&self, table: &str) -> ByteRagResult<()> {
