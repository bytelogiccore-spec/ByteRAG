...
/// 직렬화 함수 타입
pub type SerializeFn = Arc<dyn Fn(&[u8]) -> ByteRagResult<Vec<u8>> + Send + Sync>;

/// 역직렬화 함수 타입
pub type DeserializeFn = Arc<dyn Fn(&[u8]) -> ByteRagResult<Vec<u8>> + Send + Sync>;
...
impl SerializationRegistry {
    ...
    /// 데이터 역직렬화
    pub fn deserialize(&self, type_name: &str, data: &[u8]) -> ByteRagResult<Vec<u8>> {
    ...
    /// 데이터 압축 (zstd)
    pub fn compress(&self, data: &[u8], level: i32) -> ByteRagResult<Vec<u8>> {
        zstd::encode_all(data, level)
            .map_err(|e| ByteRagError::Serialization(format!("Compression failed: {}", e)))
    }

    /// 데이터 압축 해제 (zstd)
    pub fn decompress(&self, data: &[u8]) -> ByteRagResult<Vec<u8>> {
        zstd::decode_all(data)
            .map_err(|e| ByteRagError::Serialization(format!("Decompression failed: {}", e)))
    }
...
impl TwoLevelCache {
    ...
    /// 데이터 저장
    pub fn put(&self, key: String, value: Vec<u8>) -> ByteRagResult<()> {
    ...
    /// 데이터 조회
    pub fn get(&self, key: &str) -> ByteRagResult<Option<Vec<u8>>> {
        // L1 캐시 확인
    ...
    /// L2 캐시에 저장
    fn put_l2(&self, key: &str, value: &[u8]) -> ByteRagResult<()> {
        // 디렉토리 생성
    ...
    /// L2 캐시에서 조회
    fn get_l2(&self, key: &str) -> ByteRagResult<Option<Vec<u8>>> {
        let file_path = self.l2_cache_dir.join(format!("{}.bin", key));
    ...
    /// 캐시 초기화
    pub fn clear(&self) -> ByteRagResult<()> {
        // L1 캐시 초기화
