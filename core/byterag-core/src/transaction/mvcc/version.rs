...
impl VersionedKey {
    ...
    pub fn decode(bytes: &[u8]) -> ByteRagResult<Self> {
        if bytes.len() < 10 {
            return Err(ByteRagError::Storage("Invalid versioned key length".into()));
        }

        // Check magic suffix
        if bytes[bytes.len() - 2..] != [0xDB, 0x58] {
            return Err(ByteRagError::Storage("Missing MVCC magic suffix".into()));
        }
