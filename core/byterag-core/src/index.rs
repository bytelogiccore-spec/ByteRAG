...
impl HashIndex {
    ...
    ///
    /// ```rust
    /// # use byterag_core::index::HashIndex;
    /// let index = HashIndex::new();
    /// index.create_index("users", "id").unwrap();
    /// ```
    pub fn create_index(&self, table: &str, column: &str) -> ByteRagResult<()> {
    ...
    /// * `table` - Table name
    /// * `column` - Column name
    pub fn drop_index(&self, table: &str, column: &str) -> ByteRagResult<()> {
        if let Some(table_indexes) = self.indexes.get(table) {
            ...
            Err(ByteRagError::IndexNotFound {
                table: table.to_string(),
                column: column.to_string(),
            })
        }
    ...
    pub fn lookup(&self, table: &str, column: &str, value: &[u8]) -> ByteRagResult<Vec<usize>> {
        if let Some(table_indexes) = self.indexes.get(table) {
