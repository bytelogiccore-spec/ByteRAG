...
///
/// ```rust
/// use byterag_core::transaction::versionable::Versionable;
...
///     }
///
///     fn serialize(&self) -> ByteRagResult<Vec<u8>> {
...
///     }
///
///     fn deserialize(bytes: &[u8]) -> ByteRagResult<Self> {
///         if bytes.len() < 8 {
///             return Err(byterag_core::error::ByteRagError::Storage("Invalid User data".into()));
///         }
///         let id = u64::from_be_bytes(bytes[0..8].try_into().unwrap());
///         let name = String::from_utf8(bytes[8..].to_vec())
///             .map_err(|e| byterag_core::error::ByteRagError::Storage(e.to_string()))?;
///         Ok(User { id, name })
///     }
/// }
/// ```
...
impl Versionable for String {
    ...
    fn deserialize(bytes: &[u8]) -> ByteRagResult<Self> {
        String::from_utf8(bytes.to_vec())
            .map_err(|e| ByteRagError::Storage(format!("UTF-8 decode error: {}", e)))
    }
}
...
mod tests {
    ...
    impl Versionable for TestUser {
        ...
        fn deserialize(bytes: &[u8]) -> ByteRagResult<Self> {
            if bytes.len() < 16 {
                return Err(ByteRagError::Storage("Invalid TestUser data".into()));
            }
            let id = u64::from_be_bytes(bytes[0..8].try_into().unwrap());
            ...
            if bytes.len() < 16 + name_len {
                return Err(ByteRagError::Storage("Invalid TestUser name length".into()));
            }
    ...
    fn test_versionable_with_complex_data() {
        ...
        impl Versionable for ComplexData {
            ...
            fn deserialize(bytes: &[u8]) -> ByteRagResult<Self> {
                if bytes.len() < 8 {
                    return Err(ByteRagError::Storage("Invalid ComplexData".into()));
                }
                ...
                if bytes.len() < offset + 4 {
                    return Err(ByteRagError::Storage("Invalid tags count".into()));
                }
                let tags_count =
                ...
                for _ in 0..tags_count {
                    if bytes.len() < offset + 4 {
                        return Err(ByteRagError::Storage("Invalid tag length".into()));
                    }
                    let tag_len =
                        u32::from_be_bytes(bytes[offset..offset + 4].try_into().unwrap()) as usize;
                    offset += 4;
                    if bytes.len() < offset + tag_len {
                        return Err(ByteRagError::Storage("Invalid tag data".into()));
                    }
                ...
                // metadata 역직렬화
                if bytes.len() < offset + 4 {
                    return Err(ByteRagError::Storage("Invalid metadata count".into()));
                }
                let metadata_count =
                    u32::from_be_bytes(bytes[offset..offset + 4].try_into().unwrap()) as usize;
                offset += 4;
                ...
                for _ in 0..metadata_count {
                    if bytes.len() < offset + 4 {
                        return Err(ByteRagError::Storage("Invalid metadata key length".into()));
                    }
                    let k_len =
                        u32::from_be_bytes(bytes[offset..offset + 4].try_into().unwrap()) as usize;
                    offset += 4;
                    if bytes.len() < offset + k_len {
                        return Err(ByteRagError::Storage("Invalid metadata key data".into()));
                    }
                    ...
                    if bytes.len() < offset + v_len {
                        return Err(ByteRagError::Storage("Invalid metadata value data".into()));
                    }
