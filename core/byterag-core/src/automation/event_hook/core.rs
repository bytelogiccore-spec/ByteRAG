...
type EventHookActionFn =
    Box<dyn Fn(&ExecutionContext, &EventHookEvent) -> ByteRagResult<()> + Send + Sync>;
...
impl EventHook {
    ...
    pub fn fire(&self, ctx: &ExecutionContext, event: &EventHookEvent) -> ByteRagResult<bool> {
...
impl Callable for EventHook {
    fn call(&self, _ctx: &ExecutionContext, _args: &[Value]) -> ByteRagResult<Value> {
