...
use byterag_core::storage::partition::{PartitionMap, PartitionTierHint, PartitionType};

#[test]
fn test_set_and_get_tier_hint() -> ByteRagResult<()> {
...
#[test]
fn test_list_hot_partitions() -> ByteRagResult<()> {
    let db = Database::open_in_memory()?;
...
#[test]
fn test_tier_hint_default_is_hot() -> ByteRagResult<()> {
    let db = Database::open_in_memory()?;
...
#[test]
fn test_tier_hint_overwrite() -> ByteRagResult<()> {
    let db = Database::open_in_memory()?;
...
#[test]
fn test_list_partitions_empty_when_no_tier_set() -> ByteRagResult<()> {
    let db = Database::open_in_memory()?;
