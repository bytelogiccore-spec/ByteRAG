...
impl HashJoinOperator {
    ...
    fn partition_batch(
        ...
        key_indices: &[usize],
        seed: u64,
    ) -> ByteRagResult<Vec<Option<RecordBatch>>> {
    ...
    fn build_phase(&mut self) -> ByteRagResult<()> {
        ...
        if build_side_bytes > self.build_memory_budget {
            return Err(ByteRagError::Storage(format!(
    ...
    fn next_in_memory(&mut self) -> ByteRagResult<Option<RecordBatch>> {
    ...
    fn estimate_partition_size(&self, paths: &[PathBuf]) -> ByteRagResult<usize> {
        let mut total = 0;
        for path in paths {
            let meta = std::fs::metadata(path).map_err(|e| ByteRagError::Storage(e.to_string()))?;
...
fn create_column_with_nulls(
    ...
    match source_col.data_type() {
        ...
        _ => Err(ByteRagError::SqlExecution {
