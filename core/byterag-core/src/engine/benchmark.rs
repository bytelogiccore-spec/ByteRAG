...
impl BenchmarkRunner {
    ...
    /// 벤치마크 실행
    pub fn run<F>(&self, name: &str, mut f: F) -> ByteRagResult<BenchmarkResult>
    where
        F: FnMut(),
    {
    ...
    /// 베이스라인 저장
    pub fn save_baseline(&self) -> ByteRagResult<()> {
    ...
    /// 베이스라인 로드
    pub fn load_baseline(&self) -> ByteRagResult<()> {
    ...
    /// 성능 회귀 검사
    pub fn check_regression(&self, name: &str, result: &BenchmarkResult) -> ByteRagResult<()> {
        let baseline = self.baseline.read().unwrap();
        ...
        if let Some(baseline_result) = baseline.get(name) {
            ...
            if ratio > self.threshold {
                return Err(ByteRagError::PerformanceRegression {
                    name: name.to_string(),
    ...
    /// 벤치마크 실행 및 회귀 검사
    pub fn run_and_check<F>(&self, name: &str, f: F) -> ByteRagResult<BenchmarkResult>
    where
        F: FnMut(),
    {
