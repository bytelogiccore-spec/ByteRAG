...
impl ParquetWriter {
    ...
    pub fn write(path: &Path, batch: &RecordBatch) -> ByteRagResult<()> {
    ...
    pub fn write_batches(path: &Path, batches: &[RecordBatch]) -> ByteRagResult<()> {
...
impl ParquetReader {
    ...
    pub fn read(path: &Path) -> ByteRagResult<Vec<RecordBatch>> {
    ...
    pub fn read_schema(path: &Path) -> ByteRagResult<Arc<Schema>> {
        let file = File::open(path)?;
        let builder = ParquetRecordBatchReaderBuilder::try_new(file)?;
