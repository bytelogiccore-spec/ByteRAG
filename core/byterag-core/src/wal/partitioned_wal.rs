...
impl PartitionedWalWriter {
    /// 새 파티션 WAL 생성
    pub fn new(wal_dir: PathBuf, flush_threshold: usize) -> ByteRagResult<Self> {
        if !wal_dir.exists() {
            std::fs::create_dir_all(&wal_dir).map_err(|source| ByteRagError::Io { source })?;
        }
        Ok(Self {
    ...
    /// 기본 설정 (플러시 임계값: 100)
    pub fn with_defaults(wal_dir: PathBuf) -> ByteRagResult<Self> {
        Self::new(wal_dir, 100)
    }

    /// WAL 레코드 추가 (테이블별 파티션에 버퍼링)
    pub fn append(&self, record: WalRecord) -> ByteRagResult<u64> {
    ...
    pub fn append_batch(&self, records: Vec<WalRecord>) -> ByteRagResult<Vec<u64>> {
        ...
        // 병렬로 각 파티션에 추가
        let results: Vec<ByteRagResult<()>> = grouped
            .into_par_iter()
    ...
    /// 모든 파티션 플러시 (병렬)
    pub fn flush_all(&self) -> ByteRagResult<usize> {
        let tables: Vec<String> = self.partitions.iter().map(|e| e.key().clone()).collect();

        let flushed: Vec<ByteRagResult<usize>> = tables
            .par_iter()
            .map(|table| {
    ...
    /// 레코드를 디스크에 플러시
    fn flush_records(&self, table: &str, records: &[WalRecord]) -> ByteRagResult<()> {
        ...
        let mut file = std::fs::OpenOptions::new()
            ...
            .append(true)
            .open(&path)
            .map_err(|source| ByteRagError::Io { source })?;
        file.write_all(&serialized)
            .map_err(|source| ByteRagError::Io { source })?;
        file.flush().map_err(|source| ByteRagError::Io { source })?;
...
impl ParallelCheckpointManager {
    ...
    /// 여러 테이블을 병렬로 체크포인트
    pub fn checkpoint_tables(&self, tables: &[String]) -> ByteRagResult<usize> {
        let results: Vec<ByteRagResult<()>> = tables
            .par_iter()
            .map(|table| {
