...
impl DistributedExecutor {
    ...
    pub async fn execute(&self, plan: PhysicalPlan) -> ByteRagResult<Vec<RecordBatch>> {
        ...
        for stage in dag.stages {
            ...
            let timeout_secs = std::env::var("byterag_WORKER_TIMEOUT_SECS")
            ...
            for retry_count in 0..=max_retries {
                ...
                if !pending_workers.is_empty() && retry_count == max_retries {
                    ...
                    return Err(ByteRagError::Network(format!(
        ...
        let final_results = coord_task
            .await
            .map_err(|e| ByteRagError::Network(format!("Coordinator thread panic: {:?}", e)))??;
