...
pub trait Callable: Send + Sync {
    /// 함수 실행
    fn call(&self, ctx: &ExecutionContext, args: &[Value]) -> ByteRagResult<Value>;
...
impl Signature {
    /// 인자 타입 검증
    pub fn validate_args(&self, args: &[Value]) -> ByteRagResult<()> {
        if !self.is_variadic && args.len() != self.params.len() {
            return Err(ByteRagError::InvalidArguments(format!(
        ...
        for (expected, actual) in self.params.iter().zip(args.iter()) {
            if !actual.matches_type(expected) {
                return Err(ByteRagError::TypeMismatch {
...
impl Value {
    ...
    // 타입 변환 헬퍼
    pub fn as_bool(&self) -> ByteRagResult<bool> {
        match self {
            Value::Boolean(b) => Ok(*b),
            _ => Err(ByteRagError::TypeMismatch {
    ...
    pub fn as_i64(&self) -> ByteRagResult<i64> {
        match self {
            Value::Int(i) => Ok(*i),
            _ => Err(ByteRagError::TypeMismatch {
    ...
    pub fn as_f64(&self) -> ByteRagResult<f64> {
        match self {
            Value::Float(f) => Ok(*f),
            _ => Err(ByteRagError::TypeMismatch {
    ...
    pub fn as_str(&self) -> ByteRagResult<&str> {
        match self {
            Value::String(s) => Ok(s),
            _ => Err(ByteRagError::TypeMismatch {
    ...
    pub fn as_bytes(&self) -> ByteRagResult<&[u8]> {
        match self {
            Value::Bytes(b) => Ok(b),
            _ => Err(ByteRagError::TypeMismatch {
    ...
    /// 콜럼 데이터 반환
    pub fn as_array(&self) -> ByteRagResult<&Vec<Value>> {
        match self {
            Value::Array(a) => Ok(a),
            _ => Err(ByteRagError::TypeMismatch {
    ...
    /// 테이블 데이터 반환
    pub fn as_table(&self) -> ByteRagResult<&Vec<Vec<Value>>> {
        match self {
            Value::Table(t) => Ok(t),
            _ => Err(ByteRagError::TypeMismatch {
