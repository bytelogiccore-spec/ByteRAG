...
pub trait AggregateState: Send + Sync {
    /// 새 값 추가
    fn accumulate(&mut self, value: &Value) -> ByteRagResult<()>;

    /// 최종 결과 계산
    fn finalize(&self) -> ByteRagResult<Value>;
...
impl AggregateUDF {
    ...
    /// 값 배열에 대해 집계 수행
    pub fn aggregate(&self, values: &[Value]) -> ByteRagResult<Value> {
...
mod tests {
    ...
    impl AggregateState for CountState {
        fn accumulate(&mut self, _value: &Value) -> ByteRagResult<()> {
            self.count += 1;
            Ok(())
        }
