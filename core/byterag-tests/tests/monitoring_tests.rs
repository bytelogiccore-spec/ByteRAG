...
fn test_prometheus_export_format() {
    ...
    assert!(
        output.contains("# HELP byterag_inserts_total"),
        "missing HELP for inserts_total"
    );
    assert!(
        output.contains("# TYPE byterag_inserts_total counter"),
        "missing TYPE for inserts_total"
    );
    assert!(
        output.contains("byterag_inserts_total 1"),
        "inserts_total value should be 1"
    );

    assert!(
        output.contains("byterag_delta_hits_total 1"),
        "delta_hits should be 1"
    );
    assert!(
        output.contains("# TYPE byterag_delta_hit_rate gauge"),
        "should have hit rate gauge"
    );

    assert!(
        output.contains("# TYPE byterag_query_latency_us histogram"),
        "should have latency histogram"
    );
    assert!(
        output.contains("byterag_query_latency_us_count"),
        "histogram should have count"
    );
    assert!(
        output.contains("byterag_query_latency_us_sum"),
        "histogram should have sum"
    );
    assert!(
        output.contains("byterag_query_latency_us_bucket"),
