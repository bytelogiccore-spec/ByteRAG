...
impl WosPage {
    ...
    pub fn serialize(&self) -> ByteRagResult<Vec<u8>> {
        let mut buf = Vec::new();
    ...
    pub fn deserialize(bytes: &[u8]) -> ByteRagResult<Self> {
        if bytes.len() < 14 {
            return Err(ByteRagError::Storage("WosPage: too short".into()));
        }
        ...
        if crc32fast::hash(payload) != stored_crc {
            return Err(ByteRagError::Storage("WosPage: checksum mismatch".into()));
        }
