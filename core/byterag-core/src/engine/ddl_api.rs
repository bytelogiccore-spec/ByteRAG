...
impl Database {
    ...
    /// # }
    /// ```
    pub fn create_table(&self, name: &str, schema: Schema) -> ByteRagResult<()> {
    ...
    /// # }
    /// ```
    pub fn drop_table(&self, name: &str) -> ByteRagResult<()> {
    ...
    /// # }
    /// ```
    pub fn get_table_schema(&self, name: &str) -> ByteRagResult<Schema> {
        self.table_schemas
            .read()
            .unwrap()
            .get(name)
            .map(|s| (**s).clone())
            .ok_or_else(|| crate::ByteRagError::Schema(format!("Table '{}' not found", name)))
    }
    ...
    /// # }
    /// ```
    pub fn drop_sql_index(&self, table: &str, index_name: &str) -> ByteRagResult<()> {
    ...
    /// # }
    /// ```
    pub fn add_column(&self, table: &str, column_name: &str, data_type: &str) -> ByteRagResult<()> {
        let sql = format!(
    ...
    /// # }
    /// ```
    pub fn drop_column(&self, table: &str, column_name: &str) -> ByteRagResult<()> {
    ...
    /// # }
    /// ```
    pub fn rename_column(&self, table: &str, old_name: &str, new_name: &str) -> ByteRagResult<()> {
        let sql = format!(
    ...
    /// 파티셔닝 규칙을 제거합니다.
    pub fn drop_partition(&self, table: &str) -> ByteRagResult<()> {
    ...
    pub fn get_partition_stats(
        ...
        table: &str,
        partition_name: &str,
    ) -> ByteRagResult<crate::storage::partition::PartitionStats> {
        let key = format!("{}__{}", table, partition_name);
        self.partition_stats
            .get(&key)
            .map(|r| r.clone())
            .ok_or_else(|| crate::error::ByteRagError::InvalidOperation {
    ...
    pub fn all_partition_stats(
        &self,
        table: &str,
    ) -> ByteRagResult<std::collections::HashMap<String, crate::storage::partition::PartitionStats>>
    {
    ...
    /// ```rust
    /// use byterag_core::Database;
    /// use byterag_core::storage::compression::CompressionConfig;
    ///
    /// # fn main() -> byterag_core::ByteRagResult<()> {
    /// let db = Database::open_in_memory()?;
    /// // 오래된 파티션 → 고압축
    ...
    pub fn get_partition_compression(
        ...
        table: &str,
        partition_name: &str,
    ) -> ByteRagResult<crate::storage::compression::CompressionConfig> {
        let key = format!("{}__{}", table, partition_name);
        Ok(self
    ...
    pub fn enable_auto_archive(
            ...
            .is_ok()
        {
            ...
            std::thread::Builder::new()
                ...
                .map_err(|e| crate::error::ByteRagError::InvalidOperation {
    ...
    pub fn get_partition_lifecycle(
        &self,
        table: &str,
    ) -> ByteRagResult<crate::storage::partition::PartitionLifecycle> {
        self.partition_lifecycle
            .get(table)
            .map(|r| r.clone())
            .ok_or_else(|| crate::error::ByteRagError::InvalidOperation {
    ...
    /// # }
    /// ```
    pub fn run_partition_lifecycle(&self, table: &str) -> ByteRagResult<(usize, usize)> {
    ...
    pub fn run_all_partition_lifecycles(&self) -> ByteRagResult<(usize, usize)> {
    ...
    pub fn get_partition_tier(
        ...
        table: &str,
        partition_name: &str,
    ) -> ByteRagResult<crate::storage::partition::PartitionTierHint> {
        let key = format!("{}__{}", table, partition_name);
        Ok(self
    ...
    /// 뷰를 생성합니다 (Phase 5.1).
    pub fn create_view(&self, name: &str, sql: &str) -> ByteRagResult<()> {
        self.view_registry.create(name, sql)
    }

    /// 뷰를 삭제합니다 (Phase 5.1).
    pub fn drop_view(&self, name: &str) -> ByteRagResult<()> {
