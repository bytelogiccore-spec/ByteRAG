...
///
/// ```rust,no_run
/// use byterag_core::wal::encrypted_wal::EncryptedWal;
...
impl EncryptedWal {
    ...
    pub fn open(path: &Path, encryption: EncryptionConfig) -> ByteRagResult<Self> {
    ...
    fn scan_max_sequence(path: &Path, encryption: &EncryptionConfig) -> ByteRagResult<u64> {
    ...
    fn decrypt_line(line: &str, encryption: &EncryptionConfig) -> ByteRagResult<WalRecord> {
        use base64::Engine;
        use base64::engine::general_purpose::STANDARD;

        let ciphertext = STANDARD
            .decode(line.as_bytes())
            .map_err(|e| ByteRagError::Encryption(format!("base64 decode failed: {}", e)))?;

        let json_bytes = encryption.decrypt_with_aad(&ciphertext, WAL_AAD)?;

        serde_json::from_slice(&json_bytes)
            .map_err(|e| ByteRagError::Wal(format!("deserialization failed: {}", e)))
    }
    ...
    pub fn sync(&self) -> ByteRagResult<()> {
        let file = self
            .log_file
            .lock()
            .map_err(|e| ByteRagError::Wal(format!("lock failed: {}", e)))?;
    ...
    pub fn replay(&self) -> ByteRagResult<Vec<WalRecord>> {
    ...
    pub fn rekey(&mut self, new_encryption: EncryptionConfig) -> ByteRagResult<usize> {
        use base64::Engine;
        use base64::engine::general_purpose::STANDARD;
        ...
        *self
            .log_file
            .lock()
            .map_err(|e| ByteRagError::Wal(format!("lock failed: {}", e)))? = file;
