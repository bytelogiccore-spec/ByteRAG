...
**Files:**
- Modify: `core/byterag-core/src/automation/schedule.rs`
- Create: `core/byterag-core/src/automation/job_dag.rs`
- Modify: `core/byterag-core/src/automation/schedule_executor.rs`

**Step 1: 실패 테스트 작성**

```rust
// core/byterag-core/src/automation/job_dag.rs (tests)
#[test]
...
```bash
cargo test -p byterag-core -- job_dag --nocapture
```

**Step 3: 구현**

```rust
// core/byterag-core/src/automation/job_dag.rs
...
impl JobDag {
    ...
    pub fn resolve_execution_order(&self) -> ByteRagResult<Vec<String>> {
        ...
        if order.len() != self.jobs.len() {
            return Err(ByteRagError::InvalidArguments("순환 의존성 감지".into()));
        }
        Ok(order)
    }
}
...
```bash
cargo test -p byterag-core -- job_dag
```

**Step 5: Commit**

```bash
git add core/byterag-core/src/automation/job_dag.rs
...
**Files:**
- Create: `core/byterag-core/src/automation/retry_policy.rs`
- Modify: `core/byterag-core/src/automation/schedule_executor.rs`
...
fn test_retry_executes_until_success() {
    ...
    let result = policy.execute(move || {
        ...
        if a.load(Ordering::SeqCst) < 3 { Err(ByteRagError::IoError("fail".into())) }
        else { Ok(()) }
    });
    assert!(result.is_ok());
...
fn test_retry_fails_after_max() {
    ...
    let result = policy.execute::<_, ()>(|| Err(ByteRagError::IoError("always fail".into())));
...
```rust
// core/byterag-core/src/automation/retry_policy.rs
...
impl RetryPolicy {
    pub fn execute<F, T>(&self, mut f: F) -> ByteRagResult<T>
    where F: FnMut() -> ByteRagResult<T>
    {
...
```bash
cargo test -p byterag-core -- retry_policy
...
**Files:**
- Create: `core/byterag-core/src/sql/view.rs`
- Modify: `core/byterag-core/src/sql/parser/mod.rs` (CREATE/DROP VIEW 파싱)
- Modify: `core/byterag-core/src/sql/executor/mod.rs` (뷰 → 서브쿼리 확장)
- Modify: `core/byterag-core/src/engine/ddl_api.rs` (create_view, drop_view API)
...
```rust
// core/byterag-core/src/sql/view.rs
use dashmap::DashMap;
...
impl ViewRegistry {
    ...
    pub fn create(&self, name: &str, sql: &str) -> ByteRagResult<()> {
        self.views.insert(name.to_lowercase(), sql.to_string());
        Ok(())
    }

    pub fn drop(&self, name: &str) -> ByteRagResult<()> {
        self.views.remove(&name.to_lowercase())
            .map(|_| ())
            .ok_or_else(|| crate::error::ByteRagError::InvalidArguments(
                format!("뷰 '{name}' 없음")
            ))
    }
...
```bash
cargo test -p byterag-core -- view
...
**Files:**
- Create: `core/byterag-core/src/storage/partition.rs`
- Modify: `core/byterag-core/src/engine/ddl_api.rs` (`create_partition`, `drop_partition`)
- Modify: `core/byterag-core/src/engine/crud.rs` (파티션 라우팅)
- Modify: `core/byterag-core/src/sql/executor/mod.rs` (Partition Pruning)
...
```rust
// core/byterag-core/src/storage/partition.rs
#[derive(Debug, Clone)]
...
**Files:**
- Create: `core/byterag-core/src/storage/realtime_sync.rs`
- Modify: `core/byterag-core/src/engine/crud.rs` (`insert()` 후 sync 호출)
- Modify: `core/byterag-core/src/engine/parallel_engine.rs` (`DbConfig`에 `sync: RealtimeSyncConfig` 추가)

**핵심 설계**:

```rust
// core/byterag-core/src/storage/realtime_sync.rs
#[derive(Debug, Clone)]
pub enum SyncMode {
...
**Files:**
- Create: `core/byterag-core/src/engine/workload_analyzer.rs`
- Modify: `core/byterag-core/src/engine/database.rs` (백그라운드 분석 루프)
...
**Files:**
- Create: `core/byterag-core/src/replication/mod.rs`
- Create: `core/byterag-core/src/replication/master.rs`
- Create: `core/byterag-core/src/replication/slave.rs`
- Create: `core/byterag-core/src/replication/protocol.rs`
...
**Files:**
- Create: `core/byterag-core/src/sharding/mod.rs`
- Create: `core/byterag-core/src/sharding/router.rs`
- Create: `core/byterag-core/src/sharding/scatter_gather.rs`
...
```bash
# 전체 테스트
cargo test -p byterag-core

# 특정 모듈
cargo test -p byterag-core -- job_dag
cargo test -p byterag-core -- retry_policy
cargo test -p byterag-core -- view
cargo test -p byterag-core -- partition
cargo test -p byterag-core -- realtime_sync
cargo test -p byterag-core -- workload_analyzer
cargo test -p byterag-core -- replication
cargo test -p byterag-core -- sharding

# 빌드 확인
cargo build -p byterag-core
```
