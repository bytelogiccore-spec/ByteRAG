...
impl LogicalPlanner {
    ...
    pub(super) fn plan_query(&self, query: &Query) -> ByteRagResult<LogicalPlan> {
        let mut plan = match query.body.as_ref() {
            SetExpr::Select(select) => self.plan_select(select)?,
            _ => {
                return Err(ByteRagError::SqlNotSupported {
    ...
    pub(super) fn plan_select(&self, select: &Select) -> ByteRagResult<LogicalPlan> {
    ...
    pub(super) fn plan_order_by_expr(&self, ob: &SqlOrderByExpr) -> ByteRagResult<SortExpr> {
    ...
    ) -> ByteRagResult<Vec<AggregateExpr>> {
    ...
    pub(super) fn try_extract_aggregate(
        ...
        expr: &SqlExpr,
        alias: Option<String>,
    ) -> ByteRagResult<Option<AggregateExpr>> {
        match expr {
    ...
    pub(super) fn plan_function_arg(
        ...
        match args {
            ...
            sqlparser::ast::FunctionArguments::Subquery(_) => Err(ByteRagError::NotImplemented(
    ...
    pub(super) fn plan_from(&self, from: &[TableWithJoins]) -> ByteRagResult<LogicalPlan> {
        if from.is_empty() {
            return Err(ByteRagError::Schema("FROM clause is required".to_string()));
        }
    ...
    pub(super) fn plan_projection(
        &self,
        projection: &[SelectItem],
    ) -> ByteRagResult<Vec<(Expr, Option<String>)>> {
    ...
    pub(super) fn plan_expr(&self, expr: &SqlExpr) -> ByteRagResult<Expr> {
        match expr {
                ...
                let scalar = match value {
                    sqlparser::ast::Value::Number(n, _) => {
                        if let Ok(i) = n.parse::<i32>() {
                            ...
                            return Err(ByteRagError::Schema(format!("Invalid number: {}", n)));
                        }
                    }
