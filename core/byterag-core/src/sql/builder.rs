...
impl<'a, T: FromRow> Query<'a, T> {
    ...
    /// 모든 행 반환
    pub fn fetch_all(self) -> ByteRagResult<Vec<T>> {
    ...
    /// 첫 번째 행만 반환 (나머지 무시)
    pub fn fetch_first(self) -> ByteRagResult<Option<T>> {
        let final_sql = apply_params(&self.sql, &self.params, &self.named_params)?;
        let batches = self.db.execute_sql(&final_sql)?;
        for batch in &batches {
            if batch.num_rows() > 0 {
...
impl<'a, T: FromRow> QueryOne<'a, T> {
    ...
    pub fn fetch(self) -> ByteRagResult<T> {
        let final_sql = apply_params(&self.sql, &self.params, &self.named_params)?;
        let batches = self.db.execute_sql(&final_sql)?;
        let total_rows: usize = batches.iter().map(|b| b.num_rows()).sum();
        if total_rows == 0 {
            return Err(ByteRagError::KeyNotFound);
        }
        if total_rows > 1 {
            return Err(ByteRagError::InvalidOperation {
...
impl<'a, T: FromRow> QueryOptional<'a, T> {
    ...
    pub fn fetch(self) -> ByteRagResult<Option<T>> {
        let final_sql = apply_params(&self.sql, &self.params, &self.named_params)?;
        let batches = self.db.execute_sql(&final_sql)?;
        for batch in &batches {
            if batch.num_rows() > 0 {
...
impl<'a> Execute<'a> {
    ...
    pub fn run(self) -> ByteRagResult<usize> {
        let final_sql = apply_params(&self.sql, &self.params, &self.named_params)?;
        let batches = self.db.execute_sql(&final_sql)?;
...
pub trait FromScalar: Sized {
    fn from_scalar(value: &ScalarValue) -> ByteRagResult<Self>;
}
...
impl FromScalar for i64 {
    fn from_scalar(value: &ScalarValue) -> ByteRagResult<Self> {
        match value {
            ...
            _ => Err(crate::error::ByteRagError::TypeMismatch {
