...
impl ColumnarCache {
    ...
    pub fn persist_to_disk(&self, table: &str, cache_dir: &str) -> ByteRagResult<()> {
        ...
        let table_cache = self
            .tables
            .get(table)
            .ok_or_else(|| ByteRagError::Storage(format!("Table '{}' not in cache", table)))?;
        ...
        fs::create_dir_all(cache_path)
            .map_err(|e| ByteRagError::Storage(format!("Failed to create cache dir: {}", e)))?;
        ...
        for (idx, batch) in batches.iter().enumerate() {
            ...
            fs::write(&file_path, ipc_bytes)
                .map_err(|e| ByteRagError::Storage(format!("Failed to write cache file: {}", e)))?;
        }
    ...
    pub fn load_from_disk(&self, table: &str, cache_dir: &str) -> ByteRagResult<Vec<RecordBatch>> {
        ...
        loop {
            ...
            let ipc_bytes = fs::read(&file_path)
                .map_err(|e| ByteRagError::Storage(format!("Failed to read cache file: {}", e)))?;
    ...
    pub fn clear_disk_cache(&self, table: &str, cache_dir: &str) -> ByteRagResult<()> {
        use std::fs;
        use std::path::Path;
    ...
    pub fn insert_batch(&self, table: &str, batch: RecordBatch) -> ByteRagResult<()> {
    ...
    fn deserialize_arrow_ipc(value: &[u8]) -> ByteRagResult<RecordBatch> {
        let cursor = std::io::Cursor::new(value);
        let mut reader = StreamReader::try_new(cursor, None)
            .map_err(|e| ByteRagError::Serialization(format!("Arrow IPC read error: {}", e)))?;

        // Read the first (and only) batch
        reader
            .next()
            .ok_or_else(|| ByteRagError::Serialization("No batch in Arrow IPC stream".to_string()))?
            .map_err(|e| ByteRagError::Serialization(format!("Arrow IPC batch error: {}", e)))
    }
    ...
    fn sync_raw(&self, table: &str, rows: Vec<(Vec<u8>, Vec<u8>)>) -> ByteRagResult<usize> {
    ...
    pub fn get_batches_with_filter<F>(
        ...
        filter: F,
    ) -> ByteRagResult<Option<Vec<RecordBatch>>>
    where
        F: Fn(&RecordBatch) -> ByteRagResult<arrow::array::BooleanArray>,
    {
        ...
        for batch in batches.iter() {
            ...
            let filtered_batch = arrow::compute::filter_record_batch(batch, &mask)
                .map_err(|e| ByteRagError::Storage(format!("Failed to filter batch: {}", e)))?;
    ...
    pub fn clear_table(&self, table: &str) -> ByteRagResult<()> {
    ...
    pub fn clear_all(&self) -> ByteRagResult<()> {
        self.tables.clear();
...
fn project_batch(batch: &RecordBatch, indices: &[usize]) -> ByteRagResult<RecordBatch> {
    ...
    RecordBatch::try_new(projected_schema, columns)
        .map_err(|e| ByteRagError::Storage(format!("Failed to project batch: {}", e)))
}
