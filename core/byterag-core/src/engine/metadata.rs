...
impl TryFrom<SchemaMetadata> for Schema {
    type Error = ByteRagError;

    fn try_from(metadata: SchemaMetadata) -> Result<Self, Self::Error> {
        let fields: Result<Vec<Field>, ByteRagError> = metadata
            .fields
            .iter()
            .map(|field_meta| {
...
fn string_to_datatype(s: &str) -> ByteRagResult<DataType> {
    match s {
...
pub fn save_schema(wos: &NativeWosBackend, table: &str, schema: &Schema) -> ByteRagResult<()> {
    let mut metadata = SchemaMetadata::from(schema);
    metadata.table_name = table.to_string();

    let json_bytes =
        serde_json::to_vec(&metadata).map_err(|e| ByteRagError::Serialization(e.to_string()))?;
...
pub fn load_schema(wos: &NativeWosBackend, table: &str) -> ByteRagResult<Option<Arc<Schema>>> {
...
pub fn delete_schema(wos: &NativeWosBackend, table: &str) -> ByteRagResult<()> {
    wos.delete("__meta__/schemas", table.as_bytes())?;
    Ok(())
}

/// Load all schemas from persistent storage
pub fn load_all_schemas(wos: &NativeWosBackend) -> ByteRagResult<HashMap<String, Arc<Schema>>> {
    let mut schemas = HashMap::new();
    let all_records = wos.scan("__meta__/schemas", ..)?;

    for (key_vec, value_vec) in all_records {
        let table_name =
            String::from_utf8(key_vec).map_err(|e| ByteRagError::Serialization(e.to_string()))?;
...
pub fn delete_index(wos: &NativeWosBackend, index_name: &str) -> ByteRagResult<()> {
...
pub fn load_trigger(
    wos: &NativeWosBackend,
    name: &str,
) -> ByteRagResult<Option<crate::automation::Trigger>> {
...
pub fn delete_trigger(wos: &NativeWosBackend, name: &str) -> ByteRagResult<()> {
    wos.delete("__meta__/triggers", name.as_bytes())?;
    Ok(())
}

/// Load all trigger metadata from persistent storage
pub fn load_all_triggers(wos: &NativeWosBackend) -> ByteRagResult<Vec<crate::automation::Trigger>> {
    let mut triggers = Vec::new();
    let all_records = wos.scan("__meta__/triggers", ..)?;

    for (_key_vec, value_vec) in all_records {
        let json =
            String::from_utf8(value_vec).map_err(|e| ByteRagError::Serialization(e.to_string()))?;
        let trigger = crate::automation::Trigger::from_json(&json)?;
...
pub fn load_procedure(
    wos: &NativeWosBackend,
    name: &str,
) -> ByteRagResult<Option<crate::automation::StoredProcedure>> {
...
pub fn delete_procedure(wos: &NativeWosBackend, name: &str) -> ByteRagResult<()> {
...
pub fn load_all_procedures(
    wos: &NativeWosBackend,
) -> ByteRagResult<Vec<crate::automation::StoredProcedure>> {
...
pub fn save_udf(wos: &NativeWosBackend, udf: &crate::automation::UdfMetadata) -> ByteRagResult<()> {
...
pub fn load_udf(
    wos: &NativeWosBackend,
    name: &str,
) -> ByteRagResult<Option<crate::automation::UdfMetadata>> {
...
pub fn delete_udf(wos: &NativeWosBackend, name: &str) -> ByteRagResult<()> {
    wos.delete("__meta__/udfs", name.as_bytes())?;
    Ok(())
}

/// Load all UDF metadata from persistent storage
pub fn load_all_udfs(wos: &NativeWosBackend) -> ByteRagResult<Vec<crate::automation::UdfMetadata>> {
...
pub fn load_schedule(
    wos: &NativeWosBackend,
    name: &str,
) -> ByteRagResult<Option<crate::automation::Schedule>> {
    match wos.get("__meta__/schedules", name.as_bytes())? {
        Some(bytes) => {
            let json = String::from_utf8(bytes.to_vec()).map_err(|e| {
                ByteRagError::Serialization(format!("Failed to decode schedule JSON: {}", e))
            })?;
...
pub fn delete_schedule(wos: &NativeWosBackend, name: &str) -> ByteRagResult<()> {
...
pub fn load_all_schedules(
    wos: &NativeWosBackend,
) -> ByteRagResult<HashMap<String, crate::automation::Schedule>> {
